...
def escribe(cadena_original, limite = None):
    ...
    if (reportlab.__version__
        != ' $Id: geninformes.py,v 1.465 2009/11/27 10:24:44 pacoqueen Exp $ '
...
def existencias_no_nulas(hasta = None, exportar_a_csv_a = None):
    ...
    if hasta == None:
        ...
        fecha = None
        fecha_limite = " - %s, %s" % (
            utils.str_fecha(datetime.date.today()), time.strftime("%H:%M"))
    else:
        nomarchivo = os.path.join(gettempdir(),
    ...
    titulo = 'Inventario (existencias no nulas) %s' % (
                fecha_limite)
...
def existencias(hasta = None, exportar_a_csv_a = None):
    """
    Imprime un informe de existencias al día actual si "hasta" es None
    o hasta la fecha indicada.
    Esta función deja obsoleta a la anterior, que ahora se llama _existencias.
    ...
    if hasta == None:
        ...
        fecha = None
        fecha_limite = " - %s, %s" % (utils.str_fecha(datetime.date.today()),
                                      time.strftime("%H:%M"))
    else:
        ...
        fecha = utils.str_fecha(hasta)
        fecha_limite = " a %s" % (utils.str_fecha(hasta))
    titulo = 'Inventario%s' % (fecha_limite)
    campos = [('Descripción', 22),
              ('Mínimo', 7),
              ('Stock', 7),
              ('Valoración', 8),
    ...
    datos = []
    productos = pclases.ProductoCompra.select(
        pclases.ProductoCompra.q.controlExistencias == True,
        orderBy = "descripcion")
    total_valoracion = 0
    total_pendiente = 0
...
def buscar_datos_existencias(p, hasta):
    ...
    """
    uno_del_anno_corriente = datetime.date(day = 1,
                                        month = 1,
...
def _existencias(hasta = None):
    ...
    if hasta != None:
        ...
        fecha_limite = " - %s" % (utils.str_fecha(datetime.date.today()))
    cabecera(c, 'Inventario%s' % (fecha_limite))
    # El cuerpo
    ...
    if composan:
        ...
        if datos_empresa.direccion != datos_empresa.dirfacturacion:
            if not datos_empresa.esSociedad:
                ...
                c.drawString(width-2.5*inch, linea, escribe('CENTRO'))
    ...
    if lineas != None:
        for l in lineas:
            ...
            if not valorar_con_iva:
                precio_u = utils.float2str(utils._float(precio_u) / 1.18, 2)
            c.drawRightString(xprecio,
                              linea,
                              escribe(precio_u))
            total_linea = l['total']
            if not valorar_con_iva:
                total_linea = utils.float2str(utils._float(total_linea) / 1.18, 2)
...
def pedidoCompra(general, proveedor, lineas, entregas, observaciones,
    ...
    #                     escribe('ENTREGA EN NUESTRA CENTRO DE %s.' % (
...
def prefactura(cliente, factdata, lineas, arancel, vencimiento, texto,
               totales, impuesto = 1.18, orden_ventanas = None):
    """
    ...
    while (lineas != []):
        ...
        #if datos_empresa.bvqi:
        #    c.drawImage(os.path.join('..', 'imagenes',
        #       datos_empresa.logoiso2), rm-1.65*inch, height - 2.5*cm,
        #       3.3*cm, 1.85*cm)
        ...
        #    if datos_empresa.logo2 != "":
        #        c.drawImage(os.path.join('..', 'imagenes',
        #           datos_empresa.logo2), rm-1.65*inch, height - 2.5*cm,
        #           3.3*cm, 1.85*cm)
        # XXX
        ...
        # c.drawCentredString(width/2, linea,
        #   escribe('%s: %s' % (datos_empresa.irpf == 0 and "C.I.F."
        #       or "N.I.F.", datos_empresa.cif)))  # Si en datos de la empresa
        # IRPF == 0 es empresa y tiene C.I.F. Si no, considero que es
        # empresario individual y tiene N.I.F. en lugar de C.I.F.
        linea = sigLinea()
        if datos_empresa.bvqi:
            #cursiva(c, (width - c.stringWidth(
            #            escribe('Certificado marcado CE 9000122-1035'),
            #            "Courier", 10))/2 ,
            #        linea, escribe('Certificado marcado CE 9000122-1035'),
            #        "Courier", 10, (0, 0, 0), 10)
            anchotexto = c.stringWidth(
                            escribe('Certificado marcado CE 9000122-1035'),
                            "Courier", 10)
...
def trazabilidad(texto):
    ...
    while lineas:
        cabecera(c, 'Informe de trazabilidad',
                 utils.str_fecha(datetime.date.today()))
        # Marca "borrador"
        c.saveState()
...
def crm_generar_pdf_detalles_factura(factura):
    ...
    tm, bm, lm, rm = (680, 56.69, 28.35, 566.92)
    nomarchivo = os.path.join(gettempdir(),
        "crm_factura_%s_%s.pdf" % (factura.numfactura, 
                                   give_me_the_name_baby()))
    c = canvas.Canvas(nomarchivo)
    c.setPageSize(A4)
    fuente, tamanno = "Helvetica", 10
...
def texto_libre(texto, txtcabecera = "", incluir_fecha_del_dia = True):
    ...
    while lineas:
        cabecera(c, txtcabecera, incluir_fecha_del_dia
                 and utils.str_fecha(datetime.date.today()) or "")
        x, y = lm, tm + 2.5 * cm
        while y >= bm and lineas:
            linea = lineas.pop(0)
            if len(linea) > 0:
...
def listado_clientes_solo_riesgos(clientes):
    ...
    fechaini = datetime.date(day=1, month=1, year=fechafin.year)
...
def listado_clientes(clientes, titulo = None):
    """
    Imprime un listado de clientes. Los clientes a
    imprimir los recibe como lista o como SelectResult.
    """
    ...
    for cli in clientes:
        ...
        datos.append((cli.nombre,
                      ...
                      cli.telefono,
                      cli.email,
                      utils.str_fecha(cli.fechaNacimiento), 
                      cli.edad, 
                      utils.str_fecha(cli.fechaAlta), 
                      cli.sexoMasculino and "M" or "F", 
                      "; ".join([g.nombre for g in cli.get_grupos()])
                     ))
    return imprimir2(archivo, titulo, campos, datos,
                     utils.str_fecha(datetime.date.today()),
                     apaisado = True)
...
def laboratorioPartidas(datos, fecha = None):
    ...
    campos = [('NºPartida', 14), ('Longitudinal', 12), ('Transversal', 12),
              ('Compresión', 12), ('Perforación', 12), ('Permeabilidad', 16),
              ('Poros', 10), ('Espesor', 12)]
    return imprimir2(archivo, titulo, campos, datos, fecha)
...
def etiquetasRollosEtiquetadora(rollos, mostrar_marcado):
    """
    Crea etiquetas para los rollos de
    un parte de la línea de geotextil.
    Una por etiqueta del tamaño estándar de la impresora GEMINI: 12.55 x 8.4.
    """
    # Voy a tratar de reescribir esto regla en mano a ver si consigo cuadrarlo
    # bien en la etiquetadora GEMINI.
    global linea, tm, lm, rm, bm
    x, y = lm, tm
    global linea
    MAXLINEAS = 40
    ...
    for rollo in rollos:
        ...
        if mostrar_marcado and not rollo['defectuoso']:
            ...
            c.drawCentredString(width/2, height - 2.7 * cm ,
                                "9000122-1035")
        if rollo['defectuoso']:
            # DONE: If 'defectuoso' hay que quitar el marcado CE y poner las
            # medidas reales y una marca especial en la etiqueta.
            # El rollo se puede obtener a partir del ID que viene en idrollo y
            # vale != 0
            ...
            if isinstance(rollobd, pclases.Rollo):
                ...
                # fijo. Los rollos no tienen campo ancho. Es el del producto.
                rollo['peso'] = "%s kg" % (utils.float2str(prollo.peso)) # Peso
                                    # total. No teórico sin embalaje como en
                                    # las etiquetas originales.
                rollo['m2'] = "%s m²" % (utils.float2str(campos.ancho * largo))
                ...
                raise TypeError, "geninformes::etiquetasRollosEtiquetadora"\
            ...
            c.drawImage(os.path.join('..', 'imagenes', 'none.png'),
                        ...
                        width = 1 * cm,
                        height = 0.82 * cm)
            # CWT: No debe aparecer el motivo en la etiqueta. Pensaba que así
            # se identificarían mejor, pero no. No debe
            # aparecer nada que indique que el rollo es malo salvo el logotipo
            # de la "bolaspa".
            # c.setFont("Helvetica", 14)
            # el_encogedor_de_fuentes_de_doraemon(c, "Helvetica", 14, 0.2 * cm,
        ...
        c.drawString(4.0 * cm, height - 5.0 * cm,
                     escribe("Nº rollo: %s" % (rollo['nrollo'])))
        if rollo['defectuoso']:
            _dibujare_simbolitor_en_la_etiquetar(c, 9.5*cm, 0.25*cm, 2.6*cm)
        else:
            from barcode.EANBarCode import EanBarCode
            bar = EanBarCode()
            ...
            ean13rotado = ean13rotado.rotate(90)
            ean13rotado.save(nombreficheroean13)
            c.drawImage(nombreficheroean13, 10.5*cm, 0.5*cm, width = 1.5*cm)
        from barcode import code39
        codigobarras = code39.Extended39(rollo['codigo39'], xdim = 0.065 * cm)
...
def escribir_cuerpo_presupuesto(c, medidas, fuentes, presupuesto, una_linea):
    ...
    try:
        ...
        try:
            ...
            nombre = "No existen datos de la empresa"
            correoe = ""
            telefono = ""
            cargo = ""
        else:
            nombre = dde.nombreContacto
            correoe = dde.emailResponsableCompras
            telefono = dde.telefonoResponsableCompras
            cargo = ""  # FIXME: Por definir
        elabajofirmante = {
            'nombre': nombre, 
            'correoe': correoe, 
            'telefono': telefono, 
...
def informe_marcado_ce(producto,
    ...
    campos = (('(muestras)', 6),
              ...
              ('Porom.', 7))
    return imprimir2(archivo, titulo, campos, datos,
                     cols_a_derecha = range(3, 13),
                     lineas_verticales = ((29, True), ),
...
def cmr_estipulaciones(c, m, f):
    ...
    lineas_agregadas += agregarFila(m[0][0] + 0.1 * cm, m[0][1] -
        (1*cm + 0.4 * cm * lineas_agregadas), m[1][0],
    ...
    lineas_agregadas += agregarFila(m[0][0] + 0.1 * cm,
        m[0][1] - (1*cm + 0.4 * cm * lineas_agregadas), m[1][0],
...
def pruebines():
    ...
    facdata = {'facnum':_factura.numfactura,
    ...
    impuesto = None
    nomarchivo = factura(cliente, facdata, lineas, arancel, vencimiento,
                         texto, totales)
    import sys, os
    sys.path.append(os.path.join("..", "formularios"))
...
def pruebines_pales_guaje():
    sys.path.append("../formularios")
    from informes import mandar_a_imprimir_con_acrobat, abrir_pdf
    ...
    for i in range(3):
        ...
        #mandar_a_imprimir_con_acrobat(filetiqpale)
