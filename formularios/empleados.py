...
from clientes import get_pixbuf
...
class Empleados(Ventana):
    ...
    def set_foto(self, boton):
        """
        Cambia la foto seleccionada para el empleado.
        """
        ...
        if nomfich != None:
            ...
            if not self.objeto.fotos:                                     
                imagen = pclases.Foto(empleado = self.objeto, 
                                      data = imagedata)
            else:
    ...
    def comparar_campo(self, col, type_col):
        ...
        """
        res = False
        if isinstance(type_col, sqlobject.SOStringCol): # Cadena: el widget 
                                                        # es un entry
            res = self.comparar_string(col)
        elif isinstance(type_col, sqlobject.SOIntCol):  # Entero: el widget 
                                                        # es un entry
            res = self.comparar_int(col)
        elif isinstance(type_col, sqlobject.SOBoolCol): # Boolean: el widget 
                                                        # es un checkbox
            res = self.comparar_bool(col)
        elif isinstance(type_col, sqlobject.SOForeignKey):  # Entero-clave 
                                    # ajena: el widget es un comboboxentry
            res = self.comparar_ajena(col)
        elif isinstance(type_col, sqlobject.SOCol):     # Clase base, casi 
                                    # seguro Float: el widget es un entry
            res = self.comparar_float(col)
        else:
            txterror = "empleados.py: No se pudo determinar el tipo de datos"\
                       " del campo %s." % col
            print txterror
    ...
    def comparar_float(self, col):
        ...
        try:
            ...
            txt_error = "empleados.py: No se pudo convertir %s(%s) a float." % (col, valor_ventana)
            print txt_error
            self.logger.error(txt_error)
            valor_ventana = 0.0
        try:
            ...
            if not isinstance(valor_campo, type(0.0)):   # Porque es posible 
                ...
                # base. Se asume flotante por eliminación.
                res = False
            else:
    ...
    def es_diferente(self):
        ...
        for col in empleado.sqlmeta.columns:
            if col in ("planta", "nomina", "colorR", "colorG", "colorB"):
                continue
            condicion = condicion and self.comparar_campo(col, 
                                                empleado.sqlmeta.columns[col])
            if pclases.DEBUG and not condicion:
                print col, ":", getattr(empleado, col)
        # Colorines:
        for nombrecol in ("colorR", "colorG", "colorB"):
            en_pantalla = self.wids['sp_%s' % nombrecol].get_value()
            en_objeto = getattr(self.objeto, nombrecol)
            condicion = condicion and en_pantalla == en_objeto
    ...
    def rellenar_widgets(self):
        ...
        """
        numcampos = len(self.objeto.sqlmeta.columns) - 3 # HACK: Los 3 colores.
        if numcampos % 2 != 0:
        ...
        for col in self.objeto.sqlmeta.columns:
            # HACK: Es para evitar los campos de los colores, que van en 
            # una tabla aparte.
            if col in ("colorR", "colorG", "colorB"):
                continue
            # END OF HACK
        ...
        for nombrecol in ("colorR", "colorG", "colorB"):
            en_objeto = getattr(self.objeto, nombrecol)
            self.wids['sp_%s' % nombrecol].set_value(en_objeto)
        # Afote:
        try:
        ...
        for col in empleado.sqlmeta.columns:
            ...
            try:
                ...
                utils.dialogo_info(titulo = "ERROR", 
                    ...
                    padre = self.wids['ventana']) 
        # Colorines:
        for nombrecol, dbcol in (("colorR", "color_r"), 
                                 ("colorG", "color_g"), 
                                 ("colorB", "color_b")):
            en_pantalla = int(self.wids['sp_%s' % nombrecol].get_value())
            #setattr(self.objeto, nombrecol, en_pantalla)
            setattr(self.objeto, nombrecol, en_pantalla)
