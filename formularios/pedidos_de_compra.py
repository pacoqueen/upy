...
class PedidosDeCompra(Ventana):
    ...
    def crear_nuevo_pedido(self, event):
        ...
        if numpedido == None:
            ...
            if numpedido in [p.numpedido for p in peds 
                             if p.fecha 
                                and p.fecha.year==datetime.date.today().year]:
                utils.dialogo_info(titulo = 'NÚMERO DUPLICADO', 
                                   texto = "El número de pedido ya existe. No"
                                           " se pudo crear un pedido nuevo.", 
                                   padre = self.wids['ventana'])
            else:
                try:
        ...
        if pedido != None:
            try:
                ...
                utils.combo_set_from_db(self.wids['cb_proveedor'], 
                                        pedido.proveedorID)
                try:
                    self.wids['e_fecha'].set_text(utils.str_fecha(pedido.fecha))
                ...
                for ldc in pedido.lineasDeCompra:
                    ...
                    model_ldc.append([ldc.productoCompra.codigo,
                                      ...
                                      ldc.entrega,
                                      confirmada,
                                      ldc.albaranEntrada and ldc.albaranEntrada.numalbaran or "", 
                                      ldc.albaranEntrada and utils.str_fecha(ldc.albaranEntrada.fecha) or "", 
                                      ldc.id])
                subtotal = self.rellenar_lineas_de_pedido_de_compra()
                self.rellenar_totales(subtotal)
    ...
    def anadir_ldc(self, widget):
        ...
        if producto == None: 
            if utils.dialogo(titulo = "BUSCAR EN MÁS PRODUCTOS", 
                             texto = "No se encontraron productos con gestión de existencias por software.\n\n¿Desea buscar entre todos los productos de compra?", 
                             padre = self.wids['ventana']):
                producto, a_buscar = self.pedir_producto(filtrar = False, txt = a_buscar)
                if producto == None:
                    return
            else:
                return
    ...
    def guardar(self, widget):
        ...
        try:
            ...
            pedido.fecha = datetime.datetime.today()
