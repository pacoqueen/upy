...
## 7 de febredo de 2007 -> Fully functional!
## 
###################################################################
## 
###################################################################
...
""", 
    ...
    def rellenar_widgets(self):
        ...
        return os.path.getsize(self.fichero_log)
    ...
    def check_log(self):
        ...
        if os.path.getsize(self.fichero_log) > self.tamanno_log:
            ...
            try:
                for linea in self.filtrar_lineas(self.log):
                    try:
                        last_iter = self.agregar_linea(self.wids['tv_datos'].get_model(), linea)
                    except (AttributeError, KeyError), e:
                        self.logger.error("logviewer::check_log -> Error al obtener el modelo del TreeView. Probablemente se produjo una entrada en el log justo cuando se cerraba la ventana: %s" % e)
                        return False
                try:
                    self.mover_a_ultima_fila(last_iter)
    ...
    def mover_a_ultima_fila(self, last_iter):
        ...
        model = self.wids['tv_datos'].get_model()
        self.wids['tv_datos'].scroll_to_cell(model.get_path(last_iter), use_align = True)
