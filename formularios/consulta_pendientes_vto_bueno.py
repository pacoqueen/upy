...
class ConsultaPendientesVtoBueno(Ventana):
    ...
    def set_inicio(self,boton):
        ...
        self.inicio = datetime.date(year = int(temp[2]), month = int(temp[1]), day = (temp[0]))
    ...
    def buscar(self,boton):
        ...
        if self.inicio == None:
            ...
            facturas = pclases.FacturaCompra.select(sqlobject.AND(
                pclases.FacturaCompra.q.fecha 
                    >= self.inicio,pclases.FacturaCompra.q.fecha <= self.fin), 
                orderBy='fecha')       
        idproveedor = utils.combo_get_value(self.wids['cmbe_proveedor'])
        if idproveedor != None:
            proveedor = pclases.Proveedor.get(
                utils.combo_get_value(self.wids['cmbe_proveedor']))
            facturas = [v for v in facturas if v.proveedorID == proveedor.id]
        self.resultado = []
        condusuario = (self.usuario == None 
                       or self.usuario.firmaTotal or self.usuario.firmaUsuario)
        #condtecnico = self.usuario == None or self.usuario.firmaTotal or self.usuario.firmaTecnico
        condtecnico = False
        condcomercial = (self.usuario == None 
                         or self.usuario.firmaTotal 
                         or self.usuario.firmaComercial)
        conddirector = (self.usuario == None 
                        or self.usuario.firmaTotal 
                        or self.usuario.firmaDirector)
        for i in facturas:
            if boton.name == "b_pendientes":
                if (condusuario 
                    and not i.vistoBuenoUsuario 
                    and i not in self.resultado):
                    self.resultado.append(i)
                if (condtecnico 
                    and not i.vistoBuenoTecnico 
                    and i not in self.resultado):
                    self.resultado.append(i)
                if (condcomercial 
                    and not i.vistoBuenoComercial 
                    and i not in self.resultado):
                    self.resultado.append(i)
                if (conddirector 
                    and not i.vistoBuenoDirector 
                    and i not in self.resultado):
                    self.resultado.append(i)
