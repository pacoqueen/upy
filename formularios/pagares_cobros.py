...
class PagaresCobros(Ventana):
    ...
    def dividir_pagare(self, boton):
        ...
        if utils.dialogo(titulo = "¿DIVIDIR PAGARÉ?", 
                         texto = "Se procederá a dividir el pagaré actual en dos.\nAmbos serán idénticos y tendrán como importe total la mitad del importe actual.\n\n¿Está seguro de dividir el pagaré?", 
            ...
            for cobro in original.cobros:
                nuevo_cobro = pclases.Cobro(pagareCobro = copia, 
                   ...
                   importe = cobro.importe / 2.0, 
                    observaciones = 'Pagaré con fecha ??/??/???? y vencimiento ??/??/???? (pdte. de cobro)')
    ...
    def es_diferente(self):
        ...
        pagare = self.objeto
        if pagare == None: 
            return False	# Si no hay pagare activo, devuelvo que no hay cambio respecto a la ventana
        condicion = self.wids['e_fechar'].get_text() == pagare.fechaRecepcion.strftime('%d/%m/%Y')
        ...
        condicion = condicion and ((self.wids['e_fechac'].get_text() == pagare.fechaCobro.strftime('%d/%m/%Y')) 
                                    or self.wids['e_fechac'].get_text() == "" and pagare.fechaCobro == None)
        condicion = condicion and ((self.wids['e_fechar'].get_text() == pagare.fechaRecepcion.strftime('%d/%m/%Y')) 
                                    or self.wids['e_fechar'].get_text() == "" and pagare.fechaRecepcion == None)
    ...
    def pendiente(self, w):
        ...
        if w.get_active():
            ...
            # clientes.
            self.objeto.fechaCobrado = datetime.date.today()
    ...
    def buscar_vencimiento(self, factura):
        ...
        if len(vtos_full) == 0:
            utils.dialogo_info(titulo = "FACTURA SIN VENCIMIENTOS", 
                               texto = "La factura %s no tiene vencimientos.\nPara evitar incoherencias, todo cobro o pagaré debe corresponderse con un vencimiento.\nCree los vencimientos antes de relacionar la factura con un pagaré." % (factura.numfactura), 
                               padre = self.wids['ventana'])
            return None
        vtos = [(v[0].id, 
    ...
    def drop_cobro(self, b):
        pagare = self.objeto
        txt = """
        ¿Está seguro de que desea eliminar la factura seleccionada del pagaré?          
        """
    ...
    def cambiar_fechac(self, b):
        self.wids['e_fechac'].set_text(utils.str_fecha(utils.mostrar_calendario(padre = self.wids['ventana'])))

    def cambiar_fechar(self, b):
        self.wids['e_fechar'].set_text(utils.str_fecha(utils.mostrar_calendario(padre = self.wids['ventana'])))
