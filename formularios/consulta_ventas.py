...
class ConsultaVentas(Ventana):
    ...
    def __init__(self, objeto = None, usuario = None):
        ...
        cols = (('Fecha','gobject.TYPE_STRING',False,True,False,None),
                ...
                ('Total (sin IVA)', 'gobject.TYPE_STRING', False, True, False, 
                    None), 
                ...
                ('Idlineadecompra','gobject.TYPE_INT64',False,False,False,None))
        ...
        cols = (('Producto','gobject.TYPE_STRING',False,True,True,None),
                ('Cantidad','gobject.TYPE_STRING',False,True,False,None),
                ('Total (sin IVA)', 'gobject.TYPE_STRING', False, True, False, None), 
                ('Id','gobject.TYPE_INT64',False,False,False,None))
        utils.preparar_listview(self.wids['tv_producto'], cols)
        self.wids['tv_producto'].get_column(1).get_cell_renderers()[0].set_property('xalign', 1) 
        self.wids['tv_producto'].get_column(2).get_cell_renderers()[0].set_property('xalign', 1) 
        cols = (('Cliente','gobject.TYPE_STRING',False,True,True,None),
                ('CIF','gobject.TYPE_STRING',False,True,False,None),
                ('Fecha','gobject.TYPE_STRING',False,True,False,None),
                ('Importe c/IVA','gobject.TYPE_STRING',False,True,False,None),
                ('Nº. factura','gobject.TYPE_STRING',False,True,False,None),
                ('Id','gobject.TYPE_INT64',False,False,False,None))
        utils.preparar_treeview(self.wids['tv_cliente'], cols)
        self.wids['tv_cliente'].get_column(3).get_cell_renderers()[0].set_property('xalign', 1) 
        self.wids['tv_cliente'].connect("row-activated", 
                                        self.abrir_factura_o_cliente)
        inicio = datetime.date(day = 1, 
        ...
        #        ('Facturación\n(IVA incl.)','gobject.TYPE_STRING',False,True,False,None),
        #        ('Beneficio','gobject.TYPE_STRING',False,True,False,None),
        #        ('Total facturado\n(IVA incl.)','gobject.TYPE_STRING',False,True,False,None),
        #        ('Id','gobject.TYPE_STRING',False,False,False,None))
        #utils.preparar_treeview(self.wids['tv_comercial'], cols)
        #self.wids['tv_comercial'].get_column(1).get_cell_renderers()[0].set_property('xalign', 1) 
        #self.wids['tv_comercial'].get_column(2).get_cell_renderers()[0].set_property('xalign', 1) 
        #self.wids['tv_comercial'].connect("row-activated", 
        #                                  self.abrir_factura_o_comercial)
        self.wids['ch_servicios'].set_active(True)  # Por defecto lo voy a 
            ...
            # por cliente.
        self.wids['notebook1'].get_nth_page(-1).set_property("visible", False)
        gtk.main()
    ...
    def colorear(self, tv):
        ...
        def cell_func(column, cell, model, itr, numcol):
            ...
            """
            if (model[itr][2].startswith("-") or model[itr][3].startswith("-")) and model[itr][4].startswith("-"):
    ...
    #    model = tv.get_model()
    #    id = model[path][-1]
    #    if id > 0 and model[path].parent == None:   # Es comercial.
    ...
    #            import abonos
    #            v = abonos.Abonos(fda, usuario = self.usuario)
        ...
        if id > 0 and model[path].parent == None:   # Es tarifa.
            ...
            if isinstance(producto, pclases.ProductoVenta):
                if producto.es_rollo():
                    ...
                    v = productos_de_venta_rollos.ProductosDeVentaRollos(producto)
                elif producto.es_bala() or producto.es_bigbag():
                    import productos_de_venta_balas
                    v = productos_de_venta_balas.ProductosDeVentaBalas(producto)
    ...
    def rellenar_tabla(self, items, items_abono, servicios):
        ...
        for i in items_abono['lineasDeDevolucion']:
            tarifa, total = self.procesar_ldd(i, self.por_tarifa, model, total)
        for i in items_abono['lineasDeAbono']:
            tarifa, total = self.procesar_lda(i, self.por_tarifa, model, total)
    	for i in items:
            tarifa, total = self.procesar_ldv(i, self.por_tarifa, model, total)
        for srv in servicios:
            tarifa, total = self.procesar_srv(srv, self.por_tarifa, model, total)
        for tarifa in self.por_tarifa:
            ...
            if self.por_tarifa[tarifa]['kilos'] != 0:
                kilos_fibra = utils.float2str(self.por_tarifa[tarifa]['kilos'], 2)
        ...
        total_fibra_c = sum([self.por_tarifa[t]['kilos_cable'] for t in self.por_tarifa])
        if total_fibra_c:
            total_kilos += " + %s kg C" % (
    ...
    def procesar_lda(self, lda, por_tarifa, model, total):
        ...
        if False: #lda.productoVenta and lda.productoVenta.es_rollo():
            metros = lda.cantidad
            try:
                rollos = int(lda.cantidad / lda.productoVenta.camposEspecificosRollo.metros_cuadrados)
            except ZeroDivisionError:
                rollos = 0
            kilos_gtx = (metros * lda.productoVenta.camposEspecificosRollo.gramos) / 1000.0
        elif False: #lda.productoVenta and (lda.productoVenta.es_bala() or lda.productoVenta.es_bigbag()):
            kilos = lda.cantidad
        elif False: # lda.productoVenta and (lda.productoVenta.es_bala_cable()):
            kilos_cable = lda.cantidad
        else:       # ldv.producto es un pclases.ProductoCompra. No puedo medir sus metros, kilos ni bultos
            pass
        ...
        if lda.albaranSalida != None and lda.albaranSalida.cliente != None:
            ...
            cliente = "-"
        destino = lda.albaranSalida and lda.albaranSalida.nombre and lda.albaranSalida.nombre or cliente
        total = total + (precio * lda.cantidad)
        if False: # lda.productoVenta and lda.productoVenta.es_rollo():
            metros_o_kilos = "m²"
            try:
                bultos = "(%s rollos)" % (int(lda.cantidad / lda.productoVenta.camposEspecificosRollo.metros_cuadrados))
            except ZeroDivisionError:
                bultos = ""
        elif False: # lda.productoVenta and (lda.productoVenta.es_bala() or lda.productoVenta.es_bigbag()):
            metros_o_kilos = "kg"
    ...
    def procesar_ldd(self, ldd, por_tarifa, model, total):
        ...
        kilos = kilos_gtx = kilos_cable = metros = rollos = 0
        if False: # ldd.productoVenta and ldd.productoVenta.es_rollo():
            metros = ldd.cantidad
            try:
                rollos = int(ldd.cantidad / ldd.productoVenta.camposEspecificosRollo.metros_cuadrados)
            except ZeroDivisionError:
                rollos = 0
            kilos_gtx = (metros * ldd.productoVenta.camposEspecificosRollo.gramos) / 1000.0
        elif False: # ldd.productoVenta and (ldd.productoVenta.es_bala() or ldd.productoVenta.es_bigbag()):
            kilos = ldd.cantidad
        elif False: # ldd.productoVenta and (ldd.productoVenta.es_bala_cable()):
        ...
        if ldd.albaranSalida != None and ldd.albaranSalida.cliente != None:
            ...
            cliente = "-"
        destino = ldd.albaranSalida and ldd.albaranSalida.nombre and ldd.albaranSalida.nombre or cliente
        total = total + precio # * ldd.cantidad)  # El precio en la LDD ya es 
        # el subtotal. Es el precio de la unidad completa, no en m² ni kg.
        if Flase: #ldd.productoVenta and ldd.productoVenta.es_rollo():
            metros_o_kilos = "m²"
            try:
        ...
        elif False: # ldd.productoVenta and (ldd.productoVenta.es_bala() 
    ...
    def procesar_ldv(self, i, por_tarifa, model, total):
        ...
        if False: # i.productoVenta and i.productoVenta.es_rollo():
            metros = i.cantidad
            try:
                ...
                rollos = 0
            kilos_gtx = (metros * i.productoVenta.camposEspecificosRollo.gramos) / 1000.0
        elif False: # i.productoVenta and (i.productoVenta.es_bala() 
                    #              or i.productoVenta.es_bigbag()):
            kilos = i.cantidad
        elif False: # i.productoVenta and i.productoVenta.es_bala_cable():
        ...
        if False: # i.productoVenta and i.productoVenta.es_rollo():
            metros_o_kilos = "m²"
            try:
                bultos = "(%s rollos)" % (int(i.cantidad / i.productoVenta.camposEspecificosRollo.metros_cuadrados))
            except ZeroDivisionError:
                bultos = ""
            self.metros_totales += i.cantidad
        elif False: # i.productoVenta and (i.productoVenta.es_bala() or i.productoVenta.es_bigbag()):
            metros_o_kilos = "kg"
            bultos = "(N/A)"
    ...
    def procesar_srv(self, i, por_tarifa, model, total):
        ...
        if i.albaranSalida != None and i.albaranSalida.cliente != None:
            ...
            cliente = "-"
        destino = i.albaranSalida and i.albaranSalida.nombre and i.albaranSalida.nombre or cliente
        total = total + (precio * i.cantidad)
