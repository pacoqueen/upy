...
class Contadores(Ventana):
    def __init__(self, objeto = None, usuario = None):
        ...
        cols = (('Prefijo', 'gobject.TYPE_STRING', False, True, False, None),
                ('Valor actual', 'gobject.TYPE_STRING', True, True, False, self.cambiar_valor_contador),
                ('Sufijo', 'gobject.TYPE_STRING', False, True, False, None),
                ('Siguiente factura', 'gobject.TYPE_STRING', False, True, False, None),
                ('Última creada', 'gobject.TYPE_STRING', False, True, False, None),
                ('Facturas de la serie', 'gobject.TYPE_INT64', False, True, False, None),
    ...
    def rellenar_tabla(self):
        ...
        """        
        tipos = pclases.Contador.select(orderBy="prefijo")
        model = self.wids['tv_tipos'].get_model()
        model.clear()
        for t in tipos:
            model.append((t.prefijo,
                          t.contador, 
                          t.sufijo,
                          t.get_next_numfactura(commit = False), 
                          t.get_last_numfactura_creada() or "-", 
                          len(t.get_facturas()), 
                          t.id))
