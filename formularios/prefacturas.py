...
class Prefacturas(Ventana):
    ...
    def activar_widgets(self, s):
        ...
        if self.objeto and self.objeto.bloqueada and self.usuario and self.usuario.nivel >= 2:
            s = False
        ws = ('hbox1', 'hbox5', 'frame1', 'hpaned1', 'expander1', 'b_borrar', 'expander2')
        for w in ws:
        ...
        # Oculto la columna de descripción complementaria para usuarios sin permiso.
    ...
    def rellenar_widgets(self):
        ...
        # Ninguna factura debe tener un cliente None, así que si es así que salte una 
        # excepción en la siguiente línea.
    ...
    def rellenar_totales(self):
        ...
        self.wids['e_total'].set_text("%s €" % utils.float2str(self.total(subtotal, factura.cargo, tot_dto, tot_iva, abonos, tot_irpf)))
    ...
    def actualizar_vencimientos(self):
        ...
        self.rellenar_vencimientos()
        # NOTA: Si el usuario quiere que el ordenador recalcule los vencimientos que edite momentáneamente
        #       alguna LDV y pulse ENTER. Aunque no cambie el valor, se invocará el actualizar_totales y tal.
    ...
    def rellenar_ldvs(self):
        ...
        for ldv in lineasdeventa:
            ...
            model.append((ldv.producto.codigo,
                          ...
                          (ldv.albaranSalida and ldv.albaranSalida.numalbaran) \
                            or (ldv.ticket and "TICKET %d" % ldv.ticket.numticket) \
                            or '-',
        ...
        for srv in self.objeto.servicios:
            if srv.pedidoVenta == None and srv.albaranSalida != None and srv.albaranSalida.numalbaran not in peds['-']:
    ...
    def separar_ldvs(self, ldvs):
        ...
        # PLAN: Podría aprovechar y calcular aquí los totales del pedido para insertarlos en 
        #       las cabeceras del model más tarde.
        return srcs
    ...
    def rellenar_sources(self):
        ...
        # Cambios: Ya no se puede facturar nada sin albarán y además el albarán debe ser facturable.
    ...
    def rellenar_vencimientos(self):
        ...
        for vto in vtos:
            ...
            if vto[1] != None:
                ...
                # OJO: Actualizo la cantidad de la estimación a la cantidad del vencimiento real:
        ...
        if total_vtos != 0 and total_vtos != total_fra:
    ...
    def preparar_vencimientos(self):
        ...
        for lista in resto:
            ...
            while lista:
                ...
                mlc.remove(sol)
        return res
        # Creo que aunque lo comente, tampoco se va a entender. Vaya traca de algoritmo.
    ...
    def add_ldv(self, idldv):
        ...
        if ldv.prefactura == None and ldv.prefactura == None:
            ...
            if abs(ldv.cantidad_total_solicitada_del_producto - ldv.cantidad_albaraneada) > tolerancia:
                # TODO: La tolerancia es hasta que encuentre el error de cálculo que hace que aunque
                # las cantidades sean "teóricamente" iguales, no sean ==.
                txt = """
    ...
    def buscar_articulos(self):
        ...
        productos = pclases.Producto.select(pclases.OR(pclases.Producto.q.codigo.contains(a_buscar), 
    ...
    def add_vto(self, boton):
        ...
        if not estimado:
            vto = pclases.VencimientoCobro(prefactura = factura,
                                           facturaVenta = None, 
                                           fecha = datetime.date(day = fecha[0], month = fecha[1], year = fecha[2]),
                                           importe = cantidad,
    ...
    def buscar_abonos(self, w):
        ...
        filas = [(a.facturaDeAbono.id,
                  ...
                  a.numabono,
                  utils.str_fecha(a.fecha)) for a in abonos if a.facturaDeAbono != None]
...
def descontar_existencias(ldv, nueva = False):
    ...
    if control_existencias:
        factura = ldv.facturaVenta and ldv.facturaVenta or ldv.prefactura
