...
class Skel(Ventana, VentanaGenerica):
    CLASE = pclases.XXX
    VENTANA = os.path.join("..", "ui", "XXX.glade")
    ...
    def buscar(self, widget):
        ...
        if a_buscar != None:
            try:
                ...
                ida_buscar = -1
            campos_busqueda = (self.clase.q.campo1, self.clase.q.campo2) # XXX
            subsubcriterios = []
    ...
    def borrar(self, widget):
        ...
        try:
            ...
            self.logger.error("XXXskel.py::borrar -> %s ID %d no se pudo eliminar. Excepción: %s." % (self.objeto.sqlmeta.table, self.objeto.id, e))
