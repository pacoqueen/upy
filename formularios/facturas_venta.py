...
class FacturasVenta(Ventana):
    ...
    def es_diferente(self):
        ...
        try:
            condicion = condicion and (utils.parse_porcentaje(
                self.wids['e_descuento'].get_text())/100.0==factura.descuento)
            condicion = condicion and (utils.parse_porcentaje(
                self.wids['e_iva'].get_text(), fraccion = True) == factura.iva)
            condicion = condicion and (utils.parse_porcentaje(
                self.wids['e_irpf'].get_text(), fraccion = True)==factura.irpf)
    ...
    def activar_widgets(self, s):
        ...
        if (self.objeto 
            ...
            # cobros.
            self.wids['notebook1'].set_sensitive(True)
    ...
    def rellenar_widgets(self):
        ...
        """
        factura = self.objeto
        self.wids['ventana'].set_title("Facturas de venta - %s" % (factura.numfactura))
        ...
        self.wids['notebook1'].set_sensitive(cliente != None)
    ...
    def rellenar_totales(self):
        ...
        las LDVs, servicios, cargo, descuento.
        """
        factura = self.objeto
    ...
    def total(self, subtotal, cargo, dto, iva, irpf):
        return ffloat(subtotal + cargo + dto + iva + irpf)

    def total_irpf(self, base_imponible, irpf):
        return ffloat(base_imponible * irpf)

    def total_iva(self, iva, subtotal, tot_dto, cargo):
        return ffloat(subtotal + tot_dto + cargo) * iva
    ...
    def rellenar_vencimientos(self):
        ...
        for vto in vtos:
            if vto[0] != None:
                ...
                if vto[0].fecha < datetime.date.today():   # Ha vencido ya
    ...
    def preparar_vencimientos(self):
        ...
        pags = factura.cobros
        mas_larga = [l for l in (vtos, pags) if len(l)==max(len(vtos), len(pags))][0]
    ...
    def add_ldv(self, idldv):
        ...
        if ldv.facturaVenta == None and ldv.prefactura == None:
            ...
            utils.dialogo_info('VENTA FACTURADA', 'La venta ya ha sido facturada en la factura número %d.' % (ldv.factura.numfactura), 
                               padre = self.wids['ventana'])
    ...
    def crear_nueva_factura(self, widget):
        ...
        idcliente = utils.dialogo_combo('SELECCIONE CLIENTE',
                                        'Seleccione un cliente. Si no encuentra el cliente buscado, verifique\nque existe en la aplicación mediante la ventana de clientes y que no esté inhabilitado.',
                                        clientes, 
                                        padre = self.wids['ventana'])
        try:
        ...
        if numfactura_defecto == None:
            utils.dialogo_info(titulo = "SIN CONTADOR", texto = "Asigne primero un contador de facturación al cliente desde la ventana de clientes.", padre = self.wids['ventana'])
            return
        numfactura = utils.dialogo_entrada(titulo = "NÚMERO DE FACTURA", 
                                           texto = 'Introduzca un número de factura o pulse «Aceptar» para asignar el valor por defecto.\nEn cualquier caso, trate de respetar el formato del contador del cliente: %sn%s\nDonde "n" es el número entero de la factura.' % (cliente.contador.prefijo, cliente.contador.sufijo), 
                                           valor_por_defecto = numfactura_defecto, 
                                           padre = self.wids['ventana'])
        if numfactura == None: 
            return
        d, m, y = utils.mostrar_calendario(padre = self.wids['ventana'])
        fecha = datetime.date(day = d, month = m, year = y)
        ...
        if not ok:
            ...
            Factura inmediatamente posterior a la que ha intentado crear:
                ...
                - Fecha %s
            """ % (numfactura, 
                   cliente.contador.prefijo,    # Ya se ha verificado antes que el cliente tenga contador. No debería dar un attribute error.
                   ...
                   ultima_factura and ultima_factura.cliente.contador.prefijo or "",
                   ultima_factura and ultima_factura.get_numero_numfactura() or 0,
                   ultima_factura and ultima_factura.cliente.contador.sufijo or "",
                   ultima_factura and utils.str_fecha(ultima_factura.fecha) or "-",
                   factura_anterior and factura_anterior.numfactura or "-", 
                   factura_anterior and factura_anterior.cliente.contador.prefijo or "",
                   factura_anterior and factura_anterior.get_numero_numfactura() or 0,
                   factura_anterior and factura_anterior.cliente.contador.sufijo or "",
                   factura_anterior and utils.str_fecha(factura_anterior.fecha) or "-",
                   factura_posterior and factura_posterior.numfactura or "-", 
                   factura_posterior and factura_posterior.cliente.contador.prefijo or "",
                   factura_posterior and factura_posterior.get_numero_numfactura() or 0,
                   factura_posterior and factura_posterior.cliente.contador.sufijo or "",
    ...
    def chequear_restricciones_nueva_factura(self, cliente, numfactura, fecha):
        ...
        if factura_anterior:
            ok = ok and factura_anterior.get_numero_numfactura() < numero and factura_anterior.fecha <= fecha
        if factura_posterior:
            ok = ok and factura_posterior.get_numero_numfactura() > numero and factura_posterior.fecha >= fecha
    ...
    def buscar_factura(self, widget):
        ...
        a_buscar = utils.dialogo_entrada(titulo = "BUSCAR FACTURA", 
                                         texto = "Introduzca número o parte del número de factura:", 
                                         padre = self.wids['ventana']) 
        # PLAN: Ya haré para que se pueda buscar también por cliente y eso.
        if a_buscar != None:
            resultados = pclases.FacturaVenta.select(pclases.FacturaVenta.q.numfactura.contains(a_buscar), 
                                                     orderBy = "numfactura")
            if resultados.count() > 1:
    ...
    def guardar(self, widget):
        ...
        # XXX
        # CWT: Fecha es campo obligatorio. Chequeo que tenga fecha y si no se la pido ad eternum:
        # while factura.fecha == None or factura.fecha == " " or factura.fecha == "":
        #     self.cambiar_fecha(None)
        # Olvida lo de antes. Esa restricción YA ESTABA. Es imposible crear una factura sin fecha porque ¡LO PIDE AL CREARLA!
        # ¿Cómo me dice ahora que hay que hacerlo obligatorio porque sin darse cuenta creó una factura sin fecha? ¿Chi lo sa?
        # XXX
    ...
    def cambiar_numfactura(self, w):
        ...
        if nuevo_numero == None:
            utils.dialogo_info(titulo = 'NÚMERO ERRÓNEO',
                               texto = 'El número de factura %s no es correcto.\nTenga en cuenta que debe cumplir el formato impuesto por la serie asignada al cliente: %sXXX%s\n(Donde XXX representa un número entero).' % (numfactura, self.objeto.cliente.contador.prefijo, self.objeto.cliente.contador.sufijo), 
                               padre = self.wids['ventana'])
        else:
            factura_anterior, factura_posterior, ok = self.chequear_cambio_numero(self.objeto, nuevo_numero)
            if ok: 
    ...
    def activar_contenido(self, boton):
        ...
        self.wids['notebook1'].set_sensitive(idcliente != None)
        if idcliente != None:
            self.rellenar_contenido() 
    ...
    def borrar_factura(self, boton):
        ...
        # except psycopg.ProgrammingError:
            ...
            utils.dialogo_info(titulo = "ERROR", 
                               texto = "No se pudo borrar la factura por "
                                       "completo.\n\nInformación de "
                                       "depuración:\n%s" % msg, 
                               padre = self.wids['ventana'])
            ...
            try:
                ...
                self.logger.error("factura_venta.py::borrar_factura -> no se "
                                  "pudo determinar el número (numérico) de "
    ...
    def cambiar_precio_srv(self, cell, path, texto):
        ...
        srv = pclases.Servicio.get(idsrv)
        try:
            srv.precio = utils.parse_float(utils.parse_formula(texto))
    ...
    def add_vto(self, boton):
        ...
        if not estimado:
            fechaca = datetime.datetime(day = fecha[0], 
                                               month = fecha[1], 
                                               year = fecha[2])
    ...
    def cambiar_importe_cobro(self, cell, path, texto):
        ...
        if idcobro > 0:	# Es -1 si no había.
            ...
            factura = self.objeto
            cobro = pclases.Cobro(fecha = datetime.date.today(),
                                  facturaVenta = factura,
                                  importe = importe)
        self.rellenar_vencimientos()
    ...
    def crear_vencimientos_por_defecto(self, w):
        ...
        if cliente.vencimientos != None and cliente.vencimientos != '':
            ...
            if factura.fecha == None:
                factura.fecha = datetime.date.today()
            if cliente.diadepago != None and cliente.diadepago != '':
                diaest = cliente.get_dias_de_pago()
            else:
                diaest = False
            for incr in vtos:
                fechavto = factura.fecha + datetime.timedelta(days = incr)
                    ...
                    for dia_estimado in diaest:
                        ...
                        if fechaest < fechavto:     # El día estimado cae ANTES del día del vencimiento. 
                            ...
                            if mes > 12:
                                ...
                                anno += 1
                            try:
                                fechaest = datetime.datetime(day = dia_estimado, month = mes, year = anno)
                            except ValueError:
                                fechaest = datetime.datetime(day = -1, month = mes, year = anno)
    ...
    def cambiar_cantidad(self, cell, path, texto):
        ...
        if idvto > 0:	# Es -1 si no había.
            ...
            factura = self.objeto
            vto = pclases.VencimientoCobro(fecha = datetime.date.today(),
                                           facturaVenta = factura,
                                           importe = cantidad,
                                           observaciones = factura.cliente and factura.cliente.textoformacobro or "", 
                                           cuentaOrigen = factura.cliente and factura.cliente.cuentaOrigen or None)
    ...
    def cambiar_precio(self, cell, path, texto):
        ...
        model = self.wids['tv_ldvs'].get_model()
        try:
            cantidad = utils.parse_float(utils.parse_formula(texto))
        except ValueError:
...
def get_datos_remitente(usuario, padre = None):
    ...
    if not usuario or not remitente:
        ...
        if not remitente or not remitente.strip():
            utils.dialogo_info(titulo = "ENVÍO CANCELADO", 
                               texto = "Es necesaria una dirección de remite. No se enviará el correo electrónico.", 
                               padre = padre)
    ...
    if not usuario or not server:
        ...
        if not server.strip():
            utils.dialogo_info(titulo = "ENVÍO CANCELADO", 
                               texto = "Es necesario un servidor de salida. No se enviará el correo electrónico.", 
                               padre = padre)
            return remitente, smtpuser, smtppass, server
    if not usuario or not smtpuser:
        ...
        if not smtpuser.strip():
            utils.dialogo_info(titulo = "ENVÍO CANCELADO", 
                               texto = "Por favor, introduzca un usuario aunque\nel servidor no necesite autenticación para el envío.", 
                               padre = padre)
            return remitente, smtpuser, smtppass, server
            ...
            if correos:
                ...
                if not ok:
                    ...
                    utils.dialogo_info(titulo = "CORREO ELECTRÓNICO ENVIADO", 
                                       texto = "Se envió el correo electrónico a los destinatarios.", 
                                       padre = padre)
...
if __name__ == '__main__':
    ...
    #v = FacturasVenta(usuario = pclases.Usuario.select(pclases.Usuario.q.usuario.contains("admon2"))[0])
    v = FacturasVenta()
