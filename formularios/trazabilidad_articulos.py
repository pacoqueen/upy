...
class TrazabilidadArticulos(Ventana):
    ...
    def buscar(self, b):
        ...
        if a_buscar.startswith('r') or a_buscar.startswith('R'):
            try:
                objeto = pclases.Rollo.select(pclases.Rollo.q.codigo == a_buscar)[0]
            except IndexError:
                objeto = self.buscar_rollo(a_buscar[1:])
        elif a_buscar.startswith('b') or a_buscar.startswith('B'):
            try:
                objeto = pclases.Bala.select(pclases.Bala.q.codigo == a_buscar)[0]
            except IndexError:
                objeto = self.buscar_bala(a_buscar[1:])
        elif a_buscar.upper().startswith("C-"):
            try:
                loteCem = pclases.LoteCem.select(pclases.LoteCem.q.codigo == a_buscar)[0]
            except IndexError:
                utils.dialogo_info(titulo = "LOTE NO ENCONTRADO", 
                                   texto = "El lote de fibra de cemento %s no se encontró." % (a_buscar), 
                                   padre = self.wids['ventana'])
                loteCem = None
            objeto = loteCem
        elif a_buscar.upper().startswith("L-"):
            try:
                lote = pclases.Lote.select(pclases.Lote.q.numlote == int(a_buscar[2:]))[0]     
            except IndexError:
                try:
                    lote = pclases.Lote.select(pclases.Lote.q.codigo == a_buscar)[0]     
                except IndexError:
                    utils.dialogo_info(titulo = "LOTE NO ENCONTRADO", 
                                       texto = "El lote de fibra %s no se encontró." % (a_buscar), 
                                       padre = self.wids['ventana'])
                    lote = None
            except ValueError:
                utils.dialogo_info(titulo = "ERROR BUSCANDO LOTE", 
                                   texto = "El texto %s provocó un error en la búsqueda." % (a_buscar), 
                                   padre = self.wids['ventana'])
                lote = None
            objeto = lote
        elif a_buscar.upper().startswith("P-"):
            try:
                partida = pclases.Partida.select(pclases.Partida.q.numpartida == int(a_buscar[2:]))[0]
            except IndexError:
                try:
                    partida = pclases.Partida.select(pclases.Partida.q.codigo == a_buscar)[0]
                except IndexError:
                    utils.dialogo_info(titulo = "PARTIDA NO ENCONTRADA", 
                                       texto = "La partida de geotextiles %s no se encontró." % (a_buscar), 
                                       padre = self.wids['ventana'])
                    partida = None
            except ValueError:
                ...
                partida = None
            objeto = partida
        elif a_buscar.upper().startswith("PC"):
            try:
                partidacarga = pclases.PartidaCarga.select(pclases.PartidaCarga.q.numpartida == int(a_buscar[2:]))[0]
            except IndexError:
                try:
                    partidacarga = pclases.PartidaCarga.select(pclases.PartidaCarga.q.codigo == a_buscar)[0]
                except IndexError:
                    utils.dialogo_info(titulo = "PARTIDA DE CARGA NO ENCONTRADA", 
                                       texto = "La partida de carga %s no se encontró." % (a_buscar), 
                                       padre = self.wids['ventana'])
                    partidacarga = None
            except ValueError:
                ...
                partidacarga = None
            objeto = partidacarga
        elif a_buscar.startswith('c') or a_buscar.startswith('C'):
            try:
                objeto = pclases.Bigbag.select(
                    pclases.Bigbag.q.codigo == a_buscar)[0]
            except IndexError:
                objeto = self.buscar_bigbag(a_buscar[1:])
        elif a_buscar.upper().startswith('X'):
            try:
                objeto = pclases.RolloDefectuoso.select(
                    pclases.RolloDefectuoso.q.codigo == a_buscar.upper())[0]
            except IndexError:
                objeto = None       # O lo busca bien o que se vaya a "puirla". 
        elif a_buscar.upper().startswith('Z'):
            try:
                objeto = pclases.BalaCable.select(pclases.BalaCable.q.codigo == a_buscar.upper())[0]
            except IndexError:
                objeto = None       # O lo busca bien o que se vaya a "puirla". 
        elif a_buscar.upper().startswith('Y'):
            try:
                objeto = pclases.RolloC.select(pclases.RolloC.q.codigo == a_buscar.upper())[0]
            except IndexError:
                objeto = None       # O lo busca bien o que se vaya a "puirla".
        elif a_buscar.upper().startswith('H'):
            try:
                objeto = pclases.Pale.select(pclases.Pale.q.codigo == a_buscar.upper())[0]
            except IndexError:
                objeto = None       # O lo busca bien o que se vaya a "puirla".
        elif a_buscar.upper().startswith('J'):
            try:
                objeto = pclases.Caja.select(pclases.Caja.q.codigo == a_buscar.upper())[0]
            except IndexError:
                objeto = None       # O lo busca bien o que se vaya a "puirla".
        elif a_buscar.upper().startswith('K'):
        # No more bolsas como tal.
        #    try:
        #        objeto = pclases.Bolsa.select(pclases.Bolsa.q.codigo == a_buscar.upper())[0]
        #    except IndexError:
        #        objeto = None       # O lo busca bien o que se vaya a "puirla".
    ...
    def rellenar_datos(self, objeto):
        ...
        if isinstance(objeto, (pclases.Bala, pclases.Rollo, pclases.Bigbag, 
            ...
            self.wids['e_num'].set_text(objeto.codigo)
            vpro.ocultar()
        elif isinstance(objeto,(pclases.Lote,pclases.LoteCem,pclases.Partida)):
            from ventana_progreso import VentanaActividad, VentanaProgreso
            vpro = VentanaProgreso(padre = self.wids['ventana'])
            vpro.mostrar()
            i = 0.0
    ...
    def rellenar_producto_partida_carga(self, pcarga):
        ...
        for pv in prodsfibra:
            escribir(txtvw, "%s kg de %s\n" % (utils.float2str(sum([prodsfibra[pv][l]['total'] for l in prodsfibra[pv]])), 
                                                    pv.descripcion), 
                          ("negrita"))
        ...
        escribir(txtvw, "Total cargado: %s kg\n" % (utils.float2str(total)), ("negrita", "grande"))
    ...
    def rellenar_albaran_partida_carga(self, pcarga):
        ...
        for bala in pcarga.balas:
            ...
            if alb != None and alb not in albs:
                albs.append(alb)
                escribir(txtvw, "Albarán %s (%s): %s\n" % (alb.numalbaran, utils.str_fecha(alb.fecha), alb.cliente and alb.cliente.nombre or "-"))
    ...
    def rellenar_produccion_partida_carga(self, pcarga):
        ...
        escribir(txtvw, "\t%s m².\n" % (utils.float2str(total_metros)), ("negrita"))
    ...
    def rellenar_producto_lote_o_partida(self, objeto):
        ...
        if isinstance(objeto, pclases.Lote):
            for bala in objeto.balas:
                if bala.articulo and bala.articulo.productoVenta and bala.articulo.productoVenta not in productos:
                    productos.append(bala.articulo.productoVenta)
        elif isinstance(objeto, pclases.LoteCem):
            for bigbag in objeto.bigbags:
                if bigbag.articulo and bigbag.articulo.productoVenta and bigbag.articulo.productoVenta not in productos:
                    productos.append(bigbag.articulo.productoVenta)
        elif isinstance(objeto, pclases.Partida):
            for rollo in objeto.rollos:
                if rollo.articulo and rollo.articulo.productoVenta and rollo.articulo.productoVenta not in productos:
        ...
        for bulto in bultos:
            ...
            if albaran == None:
                ...
                if albaran not in albs:
                    ...
                    albs[albaran] += 1
        self.mostrar_info_albaranes_lote_o_partida(txtvw, bultos, albs, en_almacen)

    def mostrar_info_albaranes_lote_o_partida(self, txtvw, bultos, albs, en_almacen):
        """
    ...
    def mostrar_produccion_lote(self, objeto, txtvw):
        ...
        for b in objeto.balas:
            if b.articulo.parteDeProduccion != None and b.articulo.parteDeProduccion not in partes:
            ...
            if b.claseb:
                ...
                ba['bultos'] += 1
                ba['peso'] += b.pesobala
        escribir(txtvw, "Balas clase A: %d; %s kg\n" % (ba['bultos'], utils.float2str(ba['peso'])))
        escribir(txtvw, "Balas clase B: %d; %s kg\n" % (bb['bultos'], utils.float2str(bb['peso'])))
        escribir(txtvw, "TOTAL: %d balas; %s kg\n" % (ba['bultos'] + bb['bultos'], utils.float2str(ba['peso'] + bb['peso'])))
        escribir(txtvw, "\nLote de fibra fabricado en los partes:\n")
        partes.sort(lambda p1, p2: int(p1.id - p2.id))
        for parte in partes:
            escribir(txtvw, "%s (%s-%s)\n" % (utils.str_fecha(parte.fecha), utils.str_hora_corta(parte.horainicio), utils.str_hora_corta(parte.horafin)))
    ...
    def mostrar_produccion_partida(self, objeto, txtvw):
        ...
        rs = {'bultos': 0, 
              ...
              'peso_d': 0.0, 
              'peso_sin_d': 0.0, 
              'peso_teorico': objeto.get_kilos_teorico(contar_defectuosos = False),
              'peso_teorico_d': objeto.get_kilos_teorico(contar_defectuosos = True),
              'metros2_d': 0.0}
        partes = []
        for r in objeto.rollos:
            if r.articulo.parteDeProduccion != None and r.articulo.parteDeProduccion not in partes:
    ...
    def mostrar_produccion_lote_cemento(self, objeto, txtvw):
        ...
        for bb in objeto.bigbags:
            if bb.articulo.parteDeProduccion != None and bb.articulo.parteDeProduccion not in partes:
    ...
    def rellenar_info_partida(self, partida, txtvw):
        ...
        escribir(txtvw, "Código de partida: %s\n" % (partida.codigo), ("negrita"))
        escribir(txtvw, "Gramaje: %s" % (utils.float2str(partida.gramaje)))
        escribir(txtvw, " (%s)\n" % (utils.float2str(partida.calcular_gramaje_medio())), ("azul"))
        escribir(txtvw, "Resistencia longitudinal: %s" % (utils.float2str(partida.longitudinal)))
        escribir(txtvw, " (%s)\n" % (utils.float2str(partida.calcular_resistencia_longitudinal_media())), ("azul"))
        escribir(txtvw, "Alargamiento longitudinal: %s" % (utils.float2str(partida.alongitudinal)))
        escribir(txtvw, " (%s)\n" % (utils.float2str(partida.calcular_alargamiento_longitudinal_medio())), ("azul"))
        escribir(txtvw, "Resistencia transversal: %s" % (utils.float2str(partida.transversal)))
        escribir(txtvw, " (%s)\n" % (utils.float2str(partida.calcular_resistencia_transversal_media())), ("azul"))
        escribir(txtvw, "Alargamiento transversal: %s" % (utils.float2str(partida.atransversal)))
        escribir(txtvw, " (%s)\n" % (utils.float2str(partida.calcular_alargamiento_transversal_medio())), ("azul"))
        escribir(txtvw, "CBR: %s" % (utils.float2str(partida.compresion)))
        escribir(txtvw, " (%s)\n" % (utils.float2str(partida.calcular_compresion_media())), ("azul"))
        escribir(txtvw, "Perforación: %s" % (utils.float2str(partida.perforacion)))
        escribir(txtvw, " (%s)\n" % (utils.float2str(partida.calcular_perforacion_media())), ("azul"))
        escribir(txtvw, "Espesor: %s" % (utils.float2str(partida.espesor)))
        escribir(txtvw, " (%s)\n" % (utils.float2str(partida.calcular_espesor_medio())), ("azul"))
        escribir(txtvw, "Permeabilidad: %s" % (utils.float2str(partida.permeabilidad)))
        escribir(txtvw, " (%s)\n" % (utils.float2str(partida.calcular_permeabilidad_media())), ("azul"))
        escribir(txtvw, "Apertura de poros: %s" % (utils.float2str(partida.poros)))
        escribir(txtvw, " (%s)\n" % (utils.float2str(partida.calcular_poros_medio())), ("azul"))
        escribir(txtvw, "Muestras extraídas en la partida: %d\n" % len(partida.muestras))
        escribir(txtvw, "Pruebas de alargamiento longitudinal realizadas: %d\n" % len(partida.pruebasAlargamientoLongitudinal))
        escribir(txtvw, "Pruebas de alargamiento transversal realizadas: %d\n" % len(partida.pruebasAlargamientoTransversal))
        escribir(txtvw, "Pruebas de compresión realizadas: %d\n" % len(partida.pruebasCompresion))
        escribir(txtvw, "Pruebas de espesor realizadas: %d\n" % len(partida.pruebasEspesor))
        escribir(txtvw, "Pruebas de gramaje realizadas: %d\n" % len(partida.pruebasGramaje))
        escribir(txtvw, "Pruebas de perforación realizadas: %d\n" % len(partida.pruebasPerforacion))
        escribir(txtvw, "Pruebas de permeabilidad realizadas: %d\n" % len(partida.pruebasPermeabilidad))
        escribir(txtvw, "Pruebas de poros realizadas: %d\n" % len(partida.pruebasPoros))
        escribir(txtvw, "Pruebas de resistencia longitudinal realizadas: %d\n" % len(partida.pruebasResistenciaLongitudinal))
        escribir(txtvw, "Pruebas de resistencia transversal realizadas: %d\n" % len(partida.pruebasResistenciaTransversal))
    ...
    def rellenar_lotepartida(self, articulo):
        ...
        if isinstance(articulo, pclases.Bala):
            ...
            self.logger.error("trazabilidad_articulos.py::rellenar_lote_partida -> No se encontró información acerca del artículo ID %d." % (articulo.id))

    def _rellenar_albaran(self, articulo):
        """
        OJO: DEPRECATED !!!!!!
        """
        txtvw = self.wids['txt_albaran']
        borrar_texto(txtvw)
        try:
            a = articulo.articulos[0].albaranSalida
            self.mostrar_info_abonos(articulo.articulos[0])
        except IndexError, msg:
            self.logger.error("ERROR trazabilidad_articulos.py "
                              "(rellenar_albaran): %s" % (msg))
            a = None
        if a != None:
            escribir(txtvw, "Albarán número: %s\n" % a.numalbaran, ("_rojoclaro", ))
            escribir(txtvw, "Fecha: %s\n" % utils.str_fecha(a.fecha))
            escribir(txtvw, "Transportista: %s\n" % (a.transportista and a.transportista.nombre or ''))
            escribir(txtvw, "Cliente: %s\n" % (a.cliente and a.cliente.nombre or ''), "negrita")
            escribir(txtvw, "Destino: %s\n" % (a.nombre))
        if isinstance(articulo, (pclases.Rollo, pclases.RolloDefectuoso, 
                                 pclases.Bigbag, pclases.BalaCable, 
                                 pclases.RolloC)):
            #if a == None:  # DEPRECATED. Ya no se chequea así.
            ...
            if (hasattr(articulo, "parteDeProduccionID") 
                ...
                # Ahora también se pueden consumir.
                    ...
                    if isinstance(articulo, pclases.Bigbag):
                        escribir(txtvw, 
                            ...
                            "partida de fibra de cemento embolsado %s." % (
                                ...
                                pdp.partidaCem.numpartida, 
                                pdp.partidaCem.codigo), 
                                ("_rojoclaro", "cursiva"))
        elif isinstance(articulo, pclases.Bala):
            # TODO: Esto también debería estar DEPRECATED. Esperaré a llegar 
            # según la planificación para comprobarlo.
            if articulo.partida == None and a == None:
                escribir(txtvw, "El artículo está en almacén: %s\n" % (
                                articulo.articulo.almacen 
                                    and articulo.articulo.almacen.nombre 
                                    or "¡Error de coherencia en la BD!"), 
                         ("_verdeclaro", ))
            elif articulo.partida != None:
                escribir(txtvw, "Bala consumida para fabricar la partida: "
                                "%d(%s)\n" % (articulo.partida.numpartida, 
                                              articulo.partida.codigo), 
                         ("_rojoclaro", "cursiva"))
        else:
            escribir(txtvw, "ERROR DE INCONSISTENCIA. Contacte con el administrador de la base de datos.\n")
            self.logger.error("trazabilidad_articulos.py::rellenar_albaran -> Objeto ID %d no es bala, rollo ni bigbag." % (articulo.id))
    ...
    def mostrar_info_abonos(self, articulo):
        ...
        if articulo.lineasDeDevolucion:
            ...
            for ldd in ldds:
                try:
                    escribir(txtvw, "Salida del almacén el día %s en el albarán %s para %s.\n" % (
                                        utils.str_fecha(ldd.albaranSalida.fecha), 
                                        ldd.albaranSalida.numalbaran, 
                                        ldd.albaranSalida.cliente and ldd.albaranSalida.cliente.nombre or "?"), 
                                    ("_rojoclaro", "cursiva"))
                    escribir(txtvw, "Devuelto el día %s en el albarán de entrada de abono %s.\n" % (
                                        utils.str_fecha(ldd.albaranDeEntradaDeAbono.fecha), 
                                        ldd.albaranDeEntradaDeAbono.numalbaran), 
                                    ("_verdeclaro", "cursiva"))
                except AttributeError, msg:
                    escribir(txtvw, "ERROR DE INCONSISTENCIA. Contacte con el administrador de la base de datos.\n", ("negrita", ))
                    txterror = "trazabilidad_articulos.py::mostrar_info_abonos -> Excepción capturada con artículo ID %d: %s." % (
                                    articulo.id, msg)
                    print txterror
    ...
    def rellenar_produccion(self, articulo):
        ...
        if isinstance(articulo, pclases.Bala):
            ...
            escribir(txtvw, "Código de trazabilidad: %s\n" % articulo.codigo)
            escribir(txtvw, "Fecha y hora de fabricación: %s\n" % articulo.fechahora.strftime('%d/%m/%Y %H:%M'))
            escribir(txtvw, "Peso: %s\n" % (utils.float2str(articulo.pesobigbag, 1)), ("negrita",))
            escribir(txtvw, "Se extrajo muestra para laboratorio: %s\n" % (articulo.muestra and "Sí" or "No"))
            escribir(txtvw, articulo.claseb and "CLASE B\n" or "")
            escribir(txtvw, "Observaciones: %s\n" % (articulo.motivo or "-"))
        elif isinstance(articulo, (pclases.Rollo, pclases.RolloDefectuoso)):
            ...
            escribir(txtvw, "Código de trazabilidad: %s\n" % articulo.codigo)
            escribir(txtvw, "Fecha y hora de fabricación: %s\n" % articulo.fechahora.strftime('%d/%m/%Y %H:%M'))
            escribir(txtvw, "Marcado como defectuoso: %s\n" % ((isinstance(articulo, pclases.RolloDefectuoso) and "Sí") or \
                                                                     (articulo.rollob and "Sí" or "No")))
            escribir(txtvw, "Observaciones: %s\n" % articulo.observaciones)
            escribir(txtvw, "Se extrajo muestra para laboratorio: %s\n" % (hasattr(articulo, "muestra") and articulo.muestra and "Sí" or "No"))
            escribir(txtvw, "Peso: %s\n" % (utils.float2str(articulo.peso, 1)), ("negrita",))
            escribir(txtvw, "Densidad: %s\n" % (utils.float2str(articulo.densidad, 1)))
        elif isinstance(articulo, (pclases.BalaCable, pclases.RolloC)):
            escribir(txtvw, "Código de trazabilidad: %s\n" % articulo.codigo)
            escribir(txtvw, "Peso: %s\n" % (utils.float2str(articulo.peso, 1)), ("negrita",))
            escribir(txtvw, "Observaciones: %s\n" % (articulo.observaciones or "-"))
            escribir(txtvw, "Fecha y hora de embalado: %s\n" % utils.str_fechahora(articulo.fechahora))
            mostrar_parte = False   # Más que nada porque específicamente no tienen.
            ...
            self.logger.error("trazabilidad_articulos.py::rellenar_produccion -> Objeto ID %d no es de la clase bala, rollo ni bigbag." % (articulo.id))
        if mostrar_parte:
            escribir(txtvw, "\n-----------------------------------\n", ("cursiva"))
            ...
            try:
                ...
                self.logger.error("ERROR trazabilidad_articulos.py (rellenar_produccion): %s" % (msg))
                pdp = None
            except AttributeError:
                pdp = articulo.parteDeProduccion
            if pdp == None:
                escribir(txtvw, "¡No se econtró el parte de producción para la fabricación del artículo!\n", ("rojo", ))
            else:
                escribir(txtvw, "Fecha del parte: %s\n" % utils.str_fecha(pdp.fecha))
                escribir(txtvw, "Hora de inicio: %s\n" % pdp.horainicio.strftime('%H:%M'))
                escribir(txtvw, "Hora de fin: %s\n" % pdp.horafin.strftime('%H:%M'))
                escribir(txtvw, "Parte verificado y bloqueado: %s\n" % (pdp.bloqueado and "Sí" or "No"))
                escribir(txtvw, "Empleados:\n")
                for ht in pdp.horasTrabajadas:
                    escribir(txtvw, "\t%s, %s (%s)\n" % (ht.empleado.apellidos, ht.empleado.nombre, ht.horas.strftime('%H:%M')))
    ...
    def rellenar_producto(self, articulo):
        ...
        if isinstance(articulo, (pclases.Caja, pclases.Pale)):
            ...
            try:
                ...
                producto = None
        if producto == None:
            escribir(txtvw, "¡NO SE ENCONTRÓ INFORMACIÓN!\nPosible inconsistencia de la base de datos. Contacte con el administrador.")
            self.logger.error("trazabilidad_articulos.py::rellenar_producto -> Objeto %s no tiene producto asociado." % (articulo))
        else:
            escribir(txtvw, 
                          "\nCódigo de trazabilidad: %s\n" % articulo.codigo, 
                          ("negrita", ))
            try:
                codigobarras39 = code39.Extended39(articulo.codigo, xdim = .070 * cm).guardar_a_png()
                codigobarras39 = gtk.gdk.pixbuf_new_from_file(codigobarras39)
            except Exception, e:
                self.logger.error("trazabilidad_articulos::rellenar_producto -> No se pudo guardar o mostrar el código %s. Excepción: %s" % (articulo.codigo, e))
            else:
                insertar_imagen(txtvw, codigobarras39)
            if isinstance(articulo, pclases.Pale):
                codscajas = ", ".join([c.codigo for c in articulo.cajas])
                escribir(txtvw, "\nPalé de %d cajas: %s\n\n" 
                                     % (len(articulo.cajas), codscajas), 
                              ("negrita", "cursiva"))
...
def buscar_partes_fibra_fecha_y_hora(fechahora):
    ...
    """
    pdps = pclases.ParteDeProduccion.select(pclases.AND(pclases.ParteDeProduccion.q.fechahorainicio <= fechahora, 
