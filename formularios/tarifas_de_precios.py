...
class TarifasDePrecios(Ventana):
    ...
    def imprimir(self, boton):
        ...
        if datos != []:
            ...
            informes.abrir_pdf(geninformes.imprimir_tarifa(datos, nombre_tarifa, utils.str_fecha(datetime.date.today())))
...
#        utils.dialogo_info('ACTUALIZAR', 'El objeto ha sido modificado remotamente.\nDebe actualizar la información mostrada en pantalla. Pulse el botón «Actualizar».', padre = self.wids['ventana'])
    ...
    def rellenar_lista(self, wid, textos):
        ...
        if type(cb) == gtk.ComboBoxEntry:
            if cb.get_text_column()==-1:	# MORON HACK: Para evitar un "assert - GtkWarning"
    ...
    def rellenar_widgets(self):
        ...
        if tarifa!=None:
            ...
            try:
                self.wids['tv_descripcion_tarifa'].get_buffer().set_text(tarifa.observaciones)
            except TypeError:
            ...
            for p in precios:
                try:
                    porcentaje = 100*((p.precio/p.producto.preciopordefecto)-1)
                except ZeroDivisionError:
                    porcentaje = 0
                m.append([p.producto.codigo, 
                          p.producto.descripcion, 
                          p.producto.preciopordefecto, 
                          p.precio, 
                          "%s %%" % (utils.float2str(porcentaje, 1)), 
                          p.id])
            self.wids['tabla_productos'].set_model(m)
            self.wids['tabla_productos'].thaw_child_notify()
            self.wids['e_periodo_validez_ini'].set_text(utils.str_fecha(tarifa.periodoValidezIni))
            self.wids['e_periodo_validez_fin'].set_text(utils.str_fecha(tarifa.periodoValidezFin))
    ...
    def inicializar_ventana(self):
        ...
        treest = gtk.ListStore(gobject.TYPE_STRING, gobject.TYPE_STRING, gobject.TYPE_FLOAT, gobject.TYPE_FLOAT, gobject.TYPE_STRING, gobject.TYPE_INT)
    ...
    def guardar(self, button):
        ...
        ## - Identificador -> Es automático y ni lo muestro. No se puede cambiar.
