...
def dialogo_resultado(filas, 
    ...
    if tabla == -2: 
        return -2    # Cosa más fea, I can do better. 
    ...
    if maximizar:
        de.maximize()   
        # BUG: UGLY HACK: El maximize no funciona bien con beryl en Ubuntu. 
...
def redondear_flotante_en_cell_cuando_sea_posible(column, 
    ...
    if isinstance(contenido, type(1.0)):
...
def preparar_listview(tv, cols, multi = False):
    ...
    for col in cols:
        ...
        if t == 'gobject.TYPE_BOOLEAN':
            ...
            tv.insert_column_with_data_func(-1, h, cell, 
                redondear_flotante_en_cell_cuando_sea_posible, i)
            cell.set_property('xalign', 1.0)
            if o:
                columns[i].set_sort_column_id(i)
...
def dialogo_repeticion(
        titulo = "SELECCIONE LOS DÍAS QUE SE REPETIRÁ LA ACTIVIDAD", 
        padre = None):
    """
    Muestra una ventana con checkbox para seleccionar días de la semana. 
    Devuelve una lista de enteros representando los días. lunes = 0
    """
    dias = [None]
    from widgets import Widgets
    wids = Widgets('dialogo_repeticion.glade')
    wids['ventana'].set_title(titulo)
    wids['ventana'].set_transient_for(padre)
    #-------------------------------------------------------------------------#
    def aceptar(boton, dias):
        lista_valores = map(lambda x: wids[x].get_active(), 
                            ('ch_lunes', 'ch_martes', 'ch_miercoles', 
                             'ch_jueves', 'ch_viernes', 'ch_sabado', 
                             'ch_domingo'))
        _dias = []
        i = 0
    ...
    wids['b_aceptar'].connect('clicked', aceptar, dias)
    gtk.main()
    return dias[0]
...
def DateTime2DateTimeDelta(dt):
    ...
    try:
        ...
        dtd = datetime.timedelta(dt.seconds)
    else:
        m = dt.minute
        s = dt.second
...
def cmp_DateTime(f1, f2):
    """
    Compara dos fechas datetime y devuelve -1, 1 ó 0.
    Útil para ordenaciones de fechas.
    REMEMBER: ¡DEBEN SER datetime!
    """
    if f1 < f2:
        return -1
    if f1 > f2:
...
def cambiar_por_combo(tv, numcol, opts, clase, campo, ventana_padre = None, 
                      func_filtro = lambda obj, valor: True):
    """
    ...
    Para el callback de edición, si se especifica una función en func_filtro, 
    ésta debe devolver True para permitir el cambio. La función debe aceptar 
    como parámetro el objeto y el nuevo valor.
    """
    ...
    def guardar_combo(cell, path, text, model_tv, numcol, model_combo, 
        ...
        if idct == None:
            ...
            if not func_filtro(objeto_a_actualizar, valor):
                model_tv[path][numcol] = oldtext
                dialogo_info(titulo = "ERROR COMBO", 
                    texto = "No se permite la edición de este valor.", 
                    padre = ventana_padre)
                    # Jacob Nielsen, no me mates. Es que no puedo decirle al 
                    # usuario el porqué sin plantarle en la cara la función 
                    # lambda con closures y todo en la puta pantalla.
            else:
...
## Un par de utilidades de fecha.
def last_day_of(mes, anno):
    """
    Devuelve una datetime.date del último día del mes y año indicados.
    """
    import calendar
    dia = calendar.monthrange(anno, mes)[1]
    res = datetime.date(year = anno, month = mes, day = dia)
    return res

def primero_de_mes(mes = None, anno = None):
    """
    Devuelve el primero del mes y año recibido. Por defecto del mes actual.
    """
    if not mes:
        mes = datetime.date.today().month
    if not anno:
        anno = datetime.date.today().year
    return datetime.date(anno, mes, 1)

def fecha2sql(fecha):
    """
    Convierte un objeto fecha en su representación SQL-style.
    """
...
if __name__=="__main__":
    ...
    print dialogo_radio(titulo='Seleccione una opción', 
                texto='Selecciona una opción del radiobutton y tal y pam.', 
