...
import pygtk
pygtk.require('2.0')
import gtk, gtk.glade, time, sqlobject #@UnusedImport
try:
    import pclases
except ImportError:
    import sys
        ...
        if codigo != None:
            if self.objeto.proveedorID == None:
                pedidos = pclases.PedidoCompra.select(
                    pclases.PedidoCompra.q.numpedido.contains(codigo)) #@UndefinedVariable
            else:
                pedidos = pclases.PedidoCompra.select(pclases.AND(
                  pclases.PedidoCompra.q.numpedido.contains(codigo), #@UndefinedVariable
                  pclases.PedidoCompra.q.proveedorID==self.objeto.proveedorID)) #@UndefinedVariable
...
#                           padre = self.wids['ventana'])
    ...
    def ir_a_primero(self):
        ...
        try:
            ...
            albaran = pclases.AlbaranEntrada.select(
                pclases.AlbaranEntrada.q.repuestos == False, #@UndefinedVariable
                orderBy = "-id")[0]
    ...
    def refinar_resultados_busqueda(self, resultados):
        ...
        filas_res = []
        for r in resultados:
            filas_res.append((r.id, r.numalbaran, utils.str_fecha(r.fecha), r.proveedor != None and r.proveedor.nombre or ""))
    ...
    def buscar_albaran(self, widget):
        ...
        if a_buscar != None:
            resultados = pclases.AlbaranEntrada.select(pclases.AND(
                pclases.AlbaranEntrada.q.numalbaran.contains(a_buscar), #@UndefinedVariable
                pclases.AlbaranEntrada.q.repuestos == False)) #@UndefinedVariable
