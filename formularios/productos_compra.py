...
class ProductosCompra(Ventana, VentanaGenerica):
    def __init__(self, objeto = None, usuario = None):
        ...
        connections = {'b_salir/clicked': self.salir,
                       ...
                       'b_add_clase/clicked': self.add_clase, 
                       'b_drop_clase/clicked': self.drop_clase, 
    ...
    def inicializar_ventana(self):
        ...
        cols = (('# clases totales', 'gobject.TYPE_INT', True, True, False, 
                    self.cambiar_clases_totales), 
                ('Clases/mes', 'gobject.TYPE_INT', True, True, False, 
                    self.cambiar_clases_mes), 
                ('Clases/semana', 'gobject.TYPE_INT', True, True, False, 
                    self.cambiar_clases_semana), 
                ('Pax', 'gobject.TYPE_INT', True, True, False, 
                    self.cambiar_pax), 
                ('Día de la semana', 'gobject.TYPE_STRING', True, True, False, 
                    self.cambiar_dia_semana), 
                ('Máster Trainer', 'gobject.TYPE_BOOLEAN', True, True, False, 
                    self.cambiar_master_trainer), 
                ('IDClase', 'gobject.TYPE_STRING',False,False,False,None))
        utils.preparar_listview(self.wids['tv_clases'], cols, multi = True)
        # context-menu
        menu = gtk.Menu()
    ...
    def activar_widgets(self, s, chequear_permisos = True):
        ...
        ws = ('e_idproducto', 'e_codigo', 'e_descripcion',
              ...
              'tv_hist', 'tv_entradas', 'tv_salidas', 
              'table3', 'vbox2', 'cb_tipo', 'ch_control_existencias', 
              'txt_observaciones', 'b_add_clase', 'b_drop_clase') 
        for w in ws:
    ...
    def leer_numero_cell(self, texto):
        try:
            numero = utils.parse_numero(texto)
        except (TypeError, ValueError):
            utils.dialogo_info(titulo = "ERROR DE FORMATO", 
                texto = "El texto «%s» no es un número válido." % texto, 
                padre = self.wids['ventana'])
            numero = None
        return numero

    def guardar_valor_tvclase(self, texto, numcol, campo, path, minimo = None):
        """
        Lee y guarda (si es correcto) el número que viene en "texto" en el 
        campo -como cadena- del objeto y actualiza la columna del model.
        """
        model = self.wids['tv_clases'].get_model()
        numero = self.leer_numero_cell(texto)
        if numero != None:
            if minimo != None and numero < minimo:
                utils.dialogo_info(titulo = "VALOR FUERA DE RANGO", 
                    texto = "El valor %d es inferior al mínimo permitido: %d" 
                        % (numero, minimo), 
                    padre = self.wids['ventana'])
                numero = minimo
            model[path][numcol] = numero
            objeto = pclases.getObjetoPUID(model[path][-1])
            setattr(objeto, campo, numero)

    def cambiar_clases_totales(self, cell, path, texto):
        self.guardar_valor_tvclase(texto, 0, "numClasesTotales", path, 
                                   minimo = 1)

    def cambiar_clases_mes(self, cell, path, texto):
        self.guardar_valor_tvclase(texto, 1, "numClasesMes", path)

    def cambiar_clases_semana(self, cell, path, texto):
        self.guardar_valor_tvclase(texto, 2, "numClasesSemana", path)
        model = self.wids['tv_clases'].get_model()
        objeto = pclases.getObjetoPUID(model[path][-1])
        objeto.numClasesSemana = min(7, abs(objeto.numClasesSemana))
        if objeto.diaSemana != None:
            if objeto.numClasesSemana < len(objeto.diaSemana):
                objeto.diaSemana = objeto.diaSemana[:objeto.numClasesSemana]
            while objeto.numClasesSemana > len(objeto.diaSemana):
                for d in "LMXJVSD":
                    if d not in objeto.diaSemana:
                        objeto.diaSemana += d
                        break   # Y a por el siguiente
            model[path][4] = objeto.get_str_dias(objeto.diaSemana)

    def cambiar_pax(self, cell, path, texto): 
        self.guardar_valor_tvclase(texto, 3, "pax", path, minimo = 1)

    def cambiar_dia_semana(self, cell, path, texto): 
        dias = parse_dias_semana(texto)
        model = self.wids['tv_clases'].get_model()
        model[path][4] = pclases.Clase.siglas_to_dias(dias)
        objeto = pclases.getObjetoPUID(model[path][-1])
        if dias:
            objeto.diaSemana = dias
            objeto.numClasesSemana = len(dias)
            model[path][2] = objeto.numClasesSemana
        else:
            objeto.diaSemana = None

    
    def cambiar_master_trainer(self, cell, path): 
        model = self.wids['tv_clases'].get_model()
        model[path][5] = not model[path][5]
        objeto = pclases.getObjetoPUID(model[path][-1])
        objeto.masterTrainer = model[path][5]

    def add_clase(self, boton):
        clase = pclases.Clase(productoCompra = self.objeto)
        self.rellenar_clases()

    def drop_clase(self, boton):
        sel = self.wids['tv_clases'].get_selection()
    ...
    def rellenar_clases(self):
        model = self.wids['tv_clases'].get_model()
        model.clear()
        for clase in self.objeto.clases:
            model.append((clase.numClasesTotales,   
                          clase.numClasesMes, 
                          clase.numClasesSemana, 
                          clase.pax, 
                          pclases.Clase.siglas_to_dias(clase.diaSemana), 
                          clase.masterTrainer, 
                          clase.get_puid()))
    ...
    def rellenar_salidas(self, producto):
        ...
        for ldv in producto.lineasDeVenta:
            if ldv.albaranSalida or ldv.ticket:
                docsalida = (ldv.albaranSalida 
                             and ldv.albaranSalida.numalbaran 
                             or "Ticket %s" % ldv.ticket.numticket)
                try:
...
def parse_dias_semana(texto):
    """
    Devuelve las siglas en un chorizo de tipo cadena.
    -> "lunes, martes"
    <- "LM"
    """
    # TODO: l10n
    ...
    texto = texto.replace(":", " ")
    texto = texto.replace(".", " ")
    texto = texto.replace("ç", " ") # Sí, ¿qué pasa? A veces me baila el 
                                    # meñique cuando le doy a ENTER.
    texto = texto.split()
    tabla_eq = {'L': ('LUNES', 'LUN'), 
                'M': ('MARTES', 'MAR'), 
                'X': ('MIERCOLES', 'MIéRCOLES', 'MIÉRCOLES', 'MIé', 'MIÉ', 
                      'MIE'), 
                'J': ('JUEVES', 'JUE'), 
                'V': ('VIERNES', 'VIE'), 
                'S': ('SABADO', 'SáBADO', 'SÁBADO', 'SAB', 'SÁB', 'SáB'), 
                'D': ('DOMINGO', 'DOM'), 
                }
    for sigla in tabla_eq:
        if sigla in texto:
            if sigla not in cad:
                cad += sigla
                texto.remove(sigla)
        else:
            for nombredia in tabla_eq[sigla]:
                if nombredia in texto:
                    if sigla not in cad:
                        cad += sigla
                        texto.remove(nombredia)
                        break   # No sigo buscando más esta sigla.
    return cad
