...
class Colors(object):
    ...
    def parse(self, color):
        ...
        if isinstance(color, str) or isinstance(color, unicode):
            color = gtk.gdk.Color(color) #@UndefinedVariable
...
class Area(gtk.DrawingArea):
    ...
    __gsignals__ = {
        "expose-event": "override",
        "configure_event": "override",
        "mouse-over": (gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, (gobject.TYPE_PYOBJECT, )), #@UndefinedVariable
        "button-press": (gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, (gobject.TYPE_PYOBJECT, )), #@UndefinedVariable
        "button-release": (gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, (gobject.TYPE_PYOBJECT, )), #@UndefinedVariable
        "mouse-move": (gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, (gobject.TYPE_PYOBJECT, gobject.TYPE_PYOBJECT)), #@UndefinedVariable
        "mouse-click": (gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, (gobject.TYPE_PYOBJECT, gobject.TYPE_PYOBJECT)), #@UndefinedVariable
    }

    def __init__(self):
        gtk.DrawingArea.__init__(self)
        self.set_events(gtk.gdk.EXPOSURE_MASK #@UndefinedVariable
                        | gtk.gdk.LEAVE_NOTIFY_MASK #@UndefinedVariable
                        | gtk.gdk.BUTTON_PRESS_MASK #@UndefinedVariable
                        | gtk.gdk.BUTTON_RELEASE_MASK #@UndefinedVariable
                        | gtk.gdk.POINTER_MOTION_MASK #@UndefinedVariable
                        | gtk.gdk.POINTER_MOTION_HINT_MASK) #@UndefinedVariable
        self.connect("button_press_event", self.__on_button_press)
        self.connect("button_release_event", self.__on_button_release)
        self.connect("motion_notify_event", self.__on_mouse_move)
    ...
    def __on_mouse_move(self, area, event):
        ...
        if mouse_regions:
            area.window.set_cursor(gtk.gdk.Cursor(gtk.gdk.HAND2)) #@UndefinedVariable
        else:
            area.window.set_cursor(gtk.gdk.Cursor(gtk.gdk.ARROW)) #@UndefinedVariable
    ...
    def __on_button_press(self, area, event):
        x = event.x
        y = event.y
        state = event.state #@UnusedVariable
        self.mouse_drag = (x, y)
