...
class Productos(Ventana):
    ...
    def inicializar_ventana(self):
        ...
        cols = [('ID', 'gobject.TYPE_STRING', False, True, False, None), 
                ...
                ('Existencias', 'gobject.TYPE_STRING', True, True, False, self.cambiar_existencias), 
                ('Precio defecto (s/IVA)', 'gobject.TYPE_STRING', True, True, False, self.cambiar_precio_defecto)]
        self.__tarifas = [t for t in pclases.Tarifa.select(orderBy = "nombre") if t.vigente]
        ...
        for numcol in [3, 4] + [5 + i for i in range(len(self.__tarifas))]:
            ...
            if col != None:
                ...
                txt = "productos.py::inicializar_ventana -> La columna %d es None. No formateo ni hago editable en TreeView." % (numcol)
                print txt
        ...
        self.wids['e_a_buscar'].connect("key_press_event", pasar_foco_enter, self.wids['b_buscar'])
