...
###################################################################
## TODO: 
## No estaría mal poner un plot en rojo con el mínimo del producto
## cuando elija una gráfica de un solo producto.
###################################################################
...
class HistoricoExistenciasCompra(Ventana):
    ...
    def set_fecha(self, boton):
        ...
        """
        self.wids['e_fecha'].set_text(utils.str_fecha(utils.mostrar_calendario(padre = self.wids['ventana'])))
    ...
    def cambiar_grafico(self, cbe):
        ...
        if id < 0:
            ...
            if id == -1:    # Todos.
                productos = pclases.ProductoCompra.select(pclases.AND(pclases.ProductoCompra.q.existencias >0, 
                                                                      pclases.ProductoCompra.q.controlExistencias == True), 
                                                          orderBy = "descripcion")  # Es que todos, todos... no se ve nada.
            elif id == -2:  # Toda la granza.
                productos = pclases.ProductoCompra.select(pclases.AND(pclases.ProductoCompra.q.descripcion.contains("GRANZA"), 
                                                                      pclases.ProductoCompra.q.controlExistencias == True), 
                                                          orderBy = "descripcion")
            elif id == -3:  # Toda la MP.
                try:
                    matprima = pclases.TipoDeMaterial.select(pclases.TipoDeMaterial.q.descripcion.contains("prima"))[0]
                except IndexError:
                    utils.dialogo_info(titulo = "ERROR BUSCANDO TIPO DE MATERIAL", 
                                       texto = 'No se encontró el tipo de material "Materia Prima".\nSe mostrará únicamente la granza.', 
                                       padre = self.wids['ventana'])
                    productos = pclases.ProductoCompra.select(pclases.AND(pclases.ProductoCompra.q.descripcion.contains("granza"), 
                                                                          pclases.ProductoCompra.q.controlExistencias == True), 
                                                              orderBy = "descripcion")
                else:
                    productos = pclases.ProductoCompra.select(pclases.AND(pclases.ProductoCompra.q.tipoDeMaterialID == matprima.id, 
                                                                          pclases.ProductoCompra.q.controlExistencias == True), 
                                                              orderBy = "descripcion") 
            for fecha in fechas:
                ...
                for producto in productos:
                    if id == -1:
                        ...
                        datos_por_producto[producto][utils.str_fecha(fecha)] = [0, i]
                        if fecha.strftime("%d/%m/%Y") != datetime.date.today().strftime("%d/%m/%Y"):
                            datos_por_producto[producto][utils.str_fecha(fecha)][0] = producto.get_existencias_historico(fecha)
                        else:
                            datos_por_producto[producto][utils.str_fecha(fecha)][0] = producto.existencias
                    else:
                        if utils.str_fecha(fecha) not in datos:
                            datos[utils.str_fecha(fecha)] = [0, i]
                        if fecha.strftime("%d/%m/%Y") != datetime.date.today().strftime("%d/%m/%Y"):
                            datos[utils.str_fecha(fecha)][0] += producto.get_existencias_historico(fecha)
                        else:
                            datos[utils.str_fecha(fecha)][0] += producto.existencias
                act += 1
        elif id > 0:
            ...
            for fecha in fechas:
                ...
                i += 1
                if fecha.strftime("%d/%m/%Y") != datetime.date.today().strftime("%d/%m/%Y"):
                    datos[utils.str_fecha(fecha)] = [producto.get_existencias_historico(fecha), i]
                else:
                    datos[utils.str_fecha(fecha)] = [producto.existencias, i]
                act += 1
        vpro.set_valor(0.99, 'Generando gráfica...')
        while gtk.events_pending(): gtk.main_iteration(False)
        self.dibujar_grafico(datos, datos_por_producto)
        vpro.ocultar()
    ...
    def dibujar_grafica_multiple(self, datos_por_producto):
        ...
        nomarchivo = "%s.%s" % (datetime.date.today().strftime("ghistoricoprodcompra_%Y_%m_%d_%H_%M_%S"), formato)
        ...
        for producto in datos_por_producto:
            ...
            data = [ [datos[d][1], d.replace("/","//"), log( [datos[d][0] > 0 and datos[d][0] or 1][0] )] for d in datos]
            def cmp_data(d1, d2):
                return d1[0] - d2[0]
            data.sort(cmp_data)
            ...
            min_y *= 1.1
            min_global = min(0, min_y, min_global)   # Para evitar que quede un valor positivo como mínimo.
            max_global = max(1, max_y, max_global)

        xaxis = axis.X(label="Fecha")
        yaxis = axis.Y(label="Existencias", tic_interval=int(max_y) / 10)
        ar = area.T(x_coord = category_coord.T(data, 0), x_range=(-13, 0), y_range=(int(min_global), int(max_global)), x_axis = xaxis, y_axis = yaxis, size = (640, 480))
        ...
        for producto in datos_por_producto:
            ...
            ar.add_plot(line_plot.T(label = producto.descripcion.replace("/", "//")[:30], data = data))
    ...
    def dibujar_grafico(self, datos, datos_por_producto):
        """
        ...
        if datos_por_producto != {}:
            ...
            nombregraph = os.path.join(tempdir, "%s") % (nomarchivo)
            can = canvas.init(fname = nombregraph, format = formato)
            data = [[datos[d][1], d.replace("/","//"), datos[d][0]] for d in datos]
            def cmp_data(d1, d2):
                return d1[0] - d2[0]
            data.sort(cmp_data)
            data = [[i[1], i[2]] for i in data]
            ...
            min_y = min([i[1] for i in data])
            min_y *= 1.1
            min_y = min(0, min_y)   # Para evitar que quede un valor positivo como mínimo.

            xaxis = axis.X(label="Fecha")
            yaxis = axis.Y(label="Existencias", tic_interval=int(max_y) / 10)
            ar = area.T(x_coord = category_coord.T(data, 0), x_range=(-13, 0), y_range=(int(min_y), int(max_y)), x_axis = xaxis, y_axis = yaxis, size = (640, 480))
            ar.add_plot(line_plot.T(label = "existencias", data = data))
            ar.draw()
...
def get_fechas():
    """
    Devuelve una lista de fechas correspondientes 
    al día 1 de cada mes de un año atras hasta el mes actual.
    """
    fechas = []
    ultima = datetime.datetime(day = 1, month = datetime.date.today().month, year = datetime.date.today().year)
    primera = datetime.datetime(day = ultima.day, month = ultima.month, year = ultima.year -1)
    for incr_mes in xrange(12):
        mes = primera.month + incr_mes
        anno = primera.year
        try:
            fechas.append(datetime.datetime(day = primera.day, month = mes, year = anno))
        except ValueError:
            anno = primera.year + (mes / 12)
            mes = mes % 12
            fechas.append(datetime.datetime(day = primera.day, month = mes, year = anno))
    fechas.append(ultima)
    fechas.append(datetime.date.today())
    return fechas
