                    ...
                    for ldpc in i.lineasDePedidoDeCompra:
                        ...
                        if pedido:
                            ...
                            if proveedor:
                                ...
                                if nombreproveedor not in proveedores:
                                    proveedores.append(nombreproveedor)
                # pte_recibir = [ldc for ldc in i.lineasDePedidoDeCompra if ldc.albaranEntrada == None]
                codigo = i.codigo
                descripcion = i.descripcion
                if i.get_pendientes():
                    codigo = '<span background="SpringGreen"><i>%s</i></span>' % (codigo)
                    descripcion = '<span background="SpringGreen"><i>%s</i></span>' % (descripcion)
                model.append((codigo,
                              descripcion,
                              i.minimo,
                              i.existencias, 
            ...
            if numpedido != None:
                peds = pclases.PedidoCompra.select("""
                    date_part('year', fecha) = %d AND numpedido = '%s'""" % (
                        datetime.datetime.today().year, numpedido))
                if peds.count() > 0:
                    utils.dialogo_info(titulo = "DUPLICADO", 
                                       texto = "Número de pedido duplicado. Use otro.", 
                                       padre = self.wids['ventana'])
                else:
                    proveedor = get_proveedor_productos([pclases.ProductoCompra.get(model[path][-1]) for path in paths])
                    ...
                    for path in paths:
                        ...
                        ldp = pclases.LineaDePedidoDeCompra(pedidoCompra = pedido,
                                                            ...
                                                            fechaEntrega = None, 
                                                            textoEntrega = None, 
                                                            notas = "Creado a partir de la consulta de faltas.")
