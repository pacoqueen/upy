...
import pygtk
pygtk.require('2.0')
import gtk, gtk.glade, time, sqlobject, datetime
try:
    import pclases
except ImportError:
    import sys
...
class PedidosDeVenta(Ventana):
    ...
    def __init__(self, objeto = None, usuario = None):
        ...
        Ventana.__init__(self, 'pedidos_de_venta.glade', objeto, usuario = self.usuario)
        connections = {'b_nuevo/clicked': self.crear_nuevo_pedido,
                       ...
                       # 'cbe_tarifa/changed': self.cambiar_tarifa, 
                       'cbe_cliente/changed': 
    ...
    def cambiar_direccionCorrespondencia(self, combo):
        ...
        if idcliente:
            ...
            self.wids['e_nombreCorrespondencia'].set_text(cliente.nombre)
            self.wids['e_direccionCorrespondencia'].set_text(cliente.direccion)
            self.wids['e_cpCorrespondencia'].set_text(cliente.cp)
            self.wids['e_ciudadCorrespondencia'].set_text(cliente.ciudad)
            self.wids['e_provinciaCorrespondencia'].set_text(cliente.provincia)
            self.wids['e_paisCorrespondencia'].set_text(cliente.pais)
    ...
    def drop_servicio(self, widget):
        ...
        if iter == None: 
            utils.dialogo_info('SELECCIONE UN SERVICIO', 'Debe seleccionar el servicio a eliminar del pedido.', padre = self.wids['ventana'])
            return
        txt = """
        ¿Está seguro de que desea eliminar el servicio seleccionado del pedido?         
        """
        ...
        if srv.albaranSalida != None or srv.facturaVenta != None or srv.prefactura != None:
            txt =  """
        ...
        if srv.presupuesto == None:
            try:
                ...
                self.logger.error("%spedidos_de_venta.py::drop_servicio -> No se pudo eliminar el servicio ID %d." % (self.usuario and self.usuario.usuario + ": " or "", srv.id))
    ...
    def inicializar_ventana(self):
        ...
        self.wids['tv_ldvs'].get_column(4).get_cell_renderers()[0].set_property('xalign', 1.0)
        ...
        self.wids['tv_ldps'].get_column(4).get_cell_renderers()[0].set_property('xalign', 1.0)
        self.colorear()
        ...
        self.wids['tv_ldps'].connect("row-activated", self.abrir_producto_from_ldp)
        ...
        self.wids['tv_servicios'].get_column(3).get_cell_renderers()[0].set_property('xalign', 1.0)
            ...
            try:
                ...
                if 1:   #CWT: (BP) Que se ponga _siempre_ la tarifa 
                    ...
                    try:
                        ...
                        txttarifa = objtarifa and objtarifa.nombre or "sin tarifa"
                        utils.dialogo_info(titulo = "ERROR TARIFA", 
                                           texto = "La tarifa del cliente (%s) puede que no sea correcta.\nVerifique las fechas de validez de la misma y la del pedido." % txttarifa, 
                                           padre = self.wids['ventana'])
    ...
    def abrir_producto_from_ldp(self,tv, path, view_column):
        ...
        ventana = productos_compra.ProductosCompra(producto, usuario = self.usuario)
    ...
    def activar_widgets(self, s, chequear_permisos = True):
        ...
        # Aquí no se puede editar nada, dejemos que esté habilitado porque si no no se puede ni hacer scroll.
        self.wids['frame1'].set_sensitive(True)
        self.wids['frame2'].set_sensitive(True)
        self.wids['scrolledwindow3'].set_sensitive(True)
        # self.wids['tv_pendiente'].set_sensitive(True)
    ...
    def ir_a_primero(self):
        ...
        try:
            ...
            self.objeto = pclases.PedidoVenta.select(orderBy = "-id")[0] # Selecciono todos y me quedo con el primero de la lista
    ...
    def refinar_resultados_busqueda(self, resultados):
        ...
        filas_res = []
        for r in resultados:
            filas_res.append((r.id, r.numpedido, utils.str_fecha(r.fecha), r.get_nombre_cliente(), r.cerrado, r.bloqueado))
    ...
    def rellenar_widgets(self):
        ...
        if self.objeto.cliente != None:
            ...
            utils.combo_set_from_db(self.wids['cbe_cliente'], self.objeto.cliente.id)
        else:
        ...
        if self.objeto.tarifa != None:
            try:
                utils.combo_set_from_db(self.wids['cbe_tarifa'], self.objeto.tarifa.id)
            except:     # La tarifa ya no es válida para el pedido
                utils.dialogo_info(titulo = "ERROR TARIFA", 
                                   texto = "La tarifa del pedido puede que no sea correcta.\nVerifique las fechas de validez de la misma y la del pedido.", 
                                   padre = self.wids['ventana'])
                self.wids['cbe_tarifa'].set_active(-1)
                self.wids['cbe_tarifa'].child.set_text('')
        else:
        ...
        self.wids['e_total_ldvs'].set_text("%s € servidos (s/IVA)" % (utils.float2str(subtotal_servido)))
    ...
    def rellenar_desplegable_tarifas(self):
        ...
        """
        fecha_pedido = self.objeto.fecha != None and self.objeto.fecha or datetime.date.today()
    ...
    def colorear(self):
        def cell_func(column, cell, model, itr, i):
            try:
                ...
                print "LDP ID %d no encontrada. Probablemente borrada justo durante el coloreado." % (model[itr][-1])
                return
            color = None
            ...
            if isinstance(contenido, type(0.1)) and not isinstance(cell, gtk.CellRendererPixbuf):
                cell.set_property('text', (utils.float2str(contenido, 2)))
    ...
    def cambiar_cantidad_ldp(self, cell, path, texto):
        ...
        if ldp.cantidad != cantidad:
            if ldp.presupuesto != None:
                if utils.dialogo(titulo = "¿CAMBIAR CANTIDAD?", texto = "Esta línea proviene de una oferta de precio.\nCambiando la cantidad ofertada en el pedido se alteran las condiciones del presupuesto.\n\n¿Está seguro de querer cambiarlo?", padre = self.wids['ventana']):
                    ldporiginal = ldp
                    ldp = ldporiginal.clone(presupuesto = None)
                    ldporiginal.pedidoVenta = None
                else:
    ...
    def cambiar_descuento_ldp(self, cell, path, texto):
        ...
        if descuento != ldp.descuento:
            if ldp.presupuesto != None:
                if utils.dialogo(titulo = "¿CAMBIAR DESCUENTO?", texto = "Esta línea proviene de una oferta de precio.\nCambiando el descuento se alteran las condiciones del presupuesto.\n\n¿Está seguro de querer cambiar el descuento?", padre = self.wids['ventana']):
                    ldporiginal = ldp
                    ldp = ldporiginal.clone(presupuesto = None)
                    ldporiginal.pedidoVenta = None
                else:
        ...
        try:
            ...
            if texto == "":
                ...
                utils.dialogo_info(titulo = "ERROR FORMATO", 
                                   texto = "El texto %s no se corresponde con una fecha válida." % (texto), 
                                   padre = self.wids['ventana'])
                return
        model = self.wids['tv_ldps'].get_model()
        ldp = pclases.LineaDePedido.get(model[path][-1])
    ...
    def crear_nuevo_pedido(self, widget):
        ...
        """
        ultimo_pedido_mas_uno = str(pclases.PedidoVenta.get_siguiente_numero_numpedido())
        ...
        if numpedido == None:
            return
        existe = pclases.PedidoVenta.select(pclases.PedidoVenta.q.numpedido == numpedido)
        if existe.count() > 0:
            utils.dialogo_info(titulo = "PEDIDO YA EXISTE", texto = "El número de pedido ya existe. Use otro número.", padre = self.wids['ventana'])
            return
        try:
            tarifa_defecto = pclases.Tarifa.select(pclases.Tarifa.q.nombre == "Tarifa 1")[0]
        except IndexError:
            tarifa_defecto = None
        if self.objeto != None: self.objeto.notificador.desactivar()
        ...
        utils.dialogo_info('PEDIDO CREADO', 
                           'Se ha creado el pedido %s.\nComplete a continuación el resto de información acerca del mismo.' % (self.objeto.numpedido), 
                           padre = self.wids['ventana'])
    ...
    def buscar_pedido(self, widget):
        ...
        a_buscar = utils.dialogo_entrada(titulo = "BUSCAR PEDIDO", 
                                         ...
                                         padre = self.wids['ventana']) 
        if a_buscar != None:
            resultados = pclases.PedidoVenta.select(pclases.PedidoVenta.q.numpedido.contains(a_buscar))
    ...
    def guardar(self, widget):
        ...
        try:
            ...
            self.objeto.fecha = datetime.date.today()
    ...
    def cambiar_tarifa_ldv(self, boton):
        ...
        model, iter = seleccion.get_selected()
        if iter == None: 
            utils.dialogo_info('SELECCIONE UN PRODUCTO', 'Debe seleccionar el producto al que cambiar el precio.', padre = self.wids['ventana'])
            return
        idldp = model[iter][-1]
        ldp = pclases.LineaDePedido.get(idldp)
        idtarifa = utils.combo_get_value(self.wids['cbe_tarifa'])
    ...
    def drop_ldv(self, boton):
        ...
        model, iter = seleccion.get_selected()
        if iter == None: 
            utils.dialogo_info('SELECCIONE UN PRODUCTO', 'Debe seleccionar el producto a eliminar del pedido.', padre = self.wids['ventana'])
            return
        txt = """
    ...
    def borrar(self, boton):
        ...
        if not utils.dialogo('Borrar un pedido cuyas ventas estén relacionadas con albaranes de salida, facturas, etc. puede ser contraproducente.\n¿Está seguro de que desea eliminar el pedido?', 'BORRAR PEDIDO', padre = self.wids['ventana']): 
            return
#        # Primero desvinculo e intento eliminar todas las LDV y LDPs:
#        for ldv in self.objeto.lineasDeVenta:
#            ldv.pedidoVenta = None
#            ldv.eliminar()
...
#        self.objeto.destroySelf()
    ...
    def drop_ldp(self, boton):
        ...
        if ldp.albaranesSalida != []:
            ...
            albarán.
            """ % (", ".join([a and a.numalbaran or "-" for a in ldp.albaranesSalida]))
    ...
    def facturar(self, boton):
        ...
        if (not pedido.cliente 
            ...
            if utils.dialogo(titulo = "¿CREAR NUEVO PEDIDO?", 
                             texto = "¿Desea crear un nuevo pedido en "
                                     "blanco para el cliente?", 
                             padre = self.wids['ventana']):
...
def ldps_iguales(ldp1, ldp2):
    ...
    campos = ("pedidoVentaID", "productoCompraID", "precio", "fechaEntrega", "textoEntrega")
    res = True
    for c in campos:
