...
class PagaresPagos(Ventana):
    ...
    def es_diferente(self):
        ...
        condicion = condicion and ((self.wids['e_fechav'].get_text() == pagare.fechaPago.strftime('%d/%m/%Y')) 
                                    or self.wids['e_fechav'].get_text() == "" and pagare.fechaPago == None)
        condicion = condicion and ((self.wids['e_fechae'].get_text() == pagare.fechaEmision.strftime('%d/%m/%Y')) 
                                    or self.wids['e_fechae'].get_text() == "" and pagare.fechaEmision == None)
    ...
    def inicializar_ventana(self):
        ...
        """
        self.wids['b_logic'].set_property("visible", False)
        self.wids['b_pagare_monte'].set_property("visible", True)
        self.wids['b_cheque_monte'].set_property("visible", True)
        self.wids['b_pagare_caixa'].set_property("visible", True)
        self.wids['b_cheque_caixa'].set_property("visible", True)
    ...
    def ir_a_primero(self):
        ...
        try:
            ...
            pagare = None 	
        self.objeto = pagare
        self.activar_widgets(pagare != None)
        ...
        for r in resultados:
            try:
                ...
                try:
                    ...
                    proveedor = "" 
            if r.fechaPago == None or r.fechaEmision >= r.fechaPago:
                tipo = "Cheque"
            else:
                tipo = "Pagaré"
    ...
    def rellenar_widgets(self):
        ...
        if pagare.fechaPago == None or pagare.fechaEmision >= pagare.fechaPago:
            self.wids['b_caixa'].child.child.get_children()[1].set_text("Imprimir carta de cheque (laCaixa)")
        else:
            self.wids['b_caixa'].child.child.get_children()[1].set_text("Imprimir carta de pagaré (laCaixa)")
        # Dejo modificar proveedor si los pagos que tiene asociados no tienen proveedor (por ser de Logic, por ejemplo)
        ...
        if self.objeto.pagos != []:
            try:
                utils.combo_set_from_db(self.wids['cbe_proveedor'], 
                        self.objeto.pagos[0].facturaCompra.proveedor.id)
            except AttributeError:
                try:
                    utils.combo_set_from_db(self.wids['cbe_proveedor'], 
    ...
    def guardar(self, widget = None):
        ...
        if self.objeto.fechaEmision == self.objeto.fechaPago:
            while self.objeto.fechaEmision.weekday() >= 5:
                self.objeto.fechaEmision += datetime.timedelta(days = 1)
            self.objeto.fechaPago = self.objeto.fechaEmision
        while self.objeto.fechaPago.weekday() >= 5:   # NOTA: La fecha de pago debe caer entre lunes y viernes.
            self.objeto.fechaPago += datetime.timedelta(days = 1)
    ...
    def add_manual(self, boton):
        ...
        if proveedor != None:
            ...
            if observaciones != None:
                ...
                fecha = datetime.datetime(day = d, month = m, year = a)
    ...
    def rescatar_fecha_vto(self, apuntelogic): 
        ...
        if len(encontrado) != 0:    # Si hay varias fechas, el vencimiento seguramente vendrá al final de la cadena.
            fecha = encontrado[-1]
            res = datetime.datetime(day=int(fecha.split('/')[0]), 
    ...
    def add_impr_observaciones(self, docimpreso):
        txt = "\nPagado mediante %s. Imprimido el %s." % \
            (docimpreso, utils.corregir_nombres_fecha(datetime.date.today().strftime("%d de %B de %Y")))
        buffer = self.wids['txt_observaciones'].get_buffer()
    ...
    def imprimir_cheque_monte(self, boton):
        ...
        if pagare.pagos == [] or pagare.pagos[0].proveedor == None:
            ...
            if self.wids['ch_lpt'].get_active():
                london_kills_me(cantidad, receptor, euros, fechaEmision, fechaEmision, pagare = False, monte = True)
            else:
    ...
    def imprimir_pagare_monte(self, boton):
        ...
        if pagare.pagos == [] or pagare.pagos[0].proveedor == None:
            ...
            fechaEmision = pagare.fechaEmision
            if self.wids['ch_lpt'].get_active():
                london_kills_me(cantidad, receptor, euros, fechaPago, fechaEmision, pagare = True, monte = True)
            else:
        ...
        if pagare.pagos == [] or pagare.pagos[0].proveedor == None:
            ...
            if self.wids['ch_lpt'].get_active():
                london_kills_me(cantidad, receptor, euros, fechaEmision, 
                                fechaPago, pagare = True, monte = False)
            else:
                #informes.abrir_pdf(geninformes.pagareCaixa(fechaPago,cantidad,receptor,euros,fechaEmision))
                informes.abrir_pdf(geninformes.carta_pago(self.objeto, cheque = False, textofijo = self.wids['ch_imprenta'].get_active()))
    ...
    def imprimir_cheque_caixa(self, boton):
        ...
        if pagare.pagos == [] or pagare.pagos[0].proveedor == None:
            ...
            if self.wids['ch_lpt'].get_active():
                ...
                euros = numerals.numerals(cantidad, moneda = 'euros', fraccion = 'centimos')
                fechaEmision = pagare.fechaEmision
                london_kills_me(cantidad, receptor, euros, fechaEmision, fechaEmision, pagare = False, monte = False)
            else:
...
def london_kills_me(cantidad, destinatario, euros, fecha, vencimiento, pagare, monte):
    if pagare:  # ----- PAGARÉ -----
        if monte:   # ----- EL MONTE -----
            texto= build_pagare_monte(cantidad, destinatario, euros, fecha, vencimiento)
        else:       # ----- LA CAIXA -----
            texto= build_pagare_caixa(cantidad, destinatario, euros, fecha, vencimiento)
    else:       # ----- CHEQUE -----
        if monte:   # ----- EL MONTE -----
...
#    import numerals
#    euros = numerals.numerals(cantidad, moneda = '', fraccion='céntimos')  # ¿Por qué lo pondría sin moneda?
#    fecha = datetime.datetime.localtime()
#    vencimiento = fecha + datetime.timedelta(days = 7)
#    london_kills_me(cantidad, destinatario, euros, fecha, vencimiento, pagare = True, monte = True)
#    london_kills_me(cantidad, destinatario, euros, fecha, vencimiento, pagare = False, monte = True)
#    london_kills_me(cantidad, destinatario, euros, fecha, vencimiento, pagare = True, monte = False)
#    london_kills_me(cantidad, destinatario, euros, fecha, vencimiento, pagare = False, monte = False)
