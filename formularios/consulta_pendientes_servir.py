...
def buscar_pendiente_servir(cliente = None, padre = None, wids = None):
    ...
    pedidos pendientes de servir de fibra, total de fibra pendiente de servir por producto, 
    pedidos pendientes de servir de geotextiles, total de geotextiles pendientes de servir por producto.
    ...
    fibra_por_pedido = fibra_por_producto = gtx_por_pedido = gtx_por_producto = None
    #if pclases.ProductoVenta.select().count() > 0:
    #    vpro.set_valor(1/9.0, "Analizando pedidos pendientes de servir (fibra)...")
    #    fibra = build_diccionario_pendiente_servir(cliente, "fibra")
    #    vpro.set_valor(2/9.0, "Analizando pedidos pendientes de servir (geotextiles)...")
    ...
    #if pclases.ProductoVenta.select().count() > 0:
    #    vpro.set_valor(4/9.0, "Analizando pedidos pendientes de servir (fibra)...")
    #    fibra_por_pedido, raw_fibra_por_producto = build_datos_por_pedido(fibra)
    #    vpro.set_valor(5/9.0, "Analizando pedidos pendientes de servir (geotextiles)...")
    ...
    #if pclases.ProductoVenta.select().count() > 0:
    #    vpro.set_valor(7/9.0, "Analizando pedidos pendientes de servir (fibra)...")
    #    fibra_por_producto = build_datos_por_producto(raw_fibra_por_producto)
    #    vpro.set_valor(8/9.0, "Analizando pedidos pendientes de servir (geotextiles)...")
    ...
    vpro.ocultar()
    return fibra_por_pedido, fibra_por_producto, gtx_por_pedido, gtx_por_producto, otros_por_pedido, otros_por_producto
...
def build_diccionario_pendiente_servir(cliente, tipo):
    ...
    LDPs_pendientes[pedido][producto][fecha_entrega][texto_entrega] = cantidad_pendiente
    """
    LDPs_pendientes = {}    
    # Pedidos no cerrados.
    if cliente == None:
        pedidos = pclases.PedidoVenta.select(pclases.PedidoVenta.q.cerrado == False, orderBy = "numpedido")
    else:
        pedidos = pclases.PedidoVenta.select(pclases.AND(pclases.PedidoVenta.q.cerrado == False,
                                                         pclases.PedidoVenta.q.clienteID == cliente.id), orderBy = "numpedido")
    # Filtro todas las LDP y me quedo con las realmente pendientes del tipo especificado:
    for pedido in pedidos:
        ldps_pedido = [ldp for ldp in pedido.lineasDePedido 
                       if ldp.productoCompra]
        for ldp in ldps_pedido:
            ...
            if cantidad_servida < ldp.cantidad:    # Aún queda por servir
                ...
                if textoentrega not in LDPs_pendientes[pedido][producto][fechaentrega]:
