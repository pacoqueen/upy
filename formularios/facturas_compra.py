...
class FacturasDeEntrada(Ventana):
                ...
                if (ldc.albaranEntradaID != None and ldc.precio != 0 and ldc.precio != precio and utils.dialogo(titulo = "¿CAMIBAR PRECIO?", texto = txt, padre = self.wids['ventana'])) \
                   or (ldc.albaranEntradaID == None or ldc.precio == 0):    # No tiene albarán o no estaba valorado
                    ...
                    if not self.objeto.vistoBuenoAutomatico:
                        ...
                        self.objeto.fechaVistoBuenoUsuario = self.objeto.fechaVistoBuenoDirector = self.objeto.fechaVistoBuenoTecnico = self.objeto.fechaVistoBuenoComercial = datetime.date.today()
        else:
            s = pclases.ServicioTomado.get(idldc)
            s.qprecio = precio
        self.rellenar_widgets()
    ...
    def refinar_resultados_busqueda(self, resultados):
        ...
        filas_res = []
        for r in resultados:
            filas_res.append((r.id, r.numfactura, utils.str_fecha(r.fecha), r.proveedorID and r.proveedor.nombre  or "", r.numeroControl))
        idfactura = utils.dialogo_resultado(filas_res,
                                            titulo = 'SELECCIONE FACTURA',
                                            cabeceras = ('ID Interno', 'Número de factura', 'Fecha', 'Proveedor', 'Código control'), 
                                            padre = self.wids['ventana']) 
        if idfactura < 0:
            return None
        else:
    ...
    def rellenar_vistos_buenos(self):
        ...
        if vto_bueno_auto:
            ...
            if factura.fechaVistoBuenoComercial == None:
                factura.fechaVistoBuenoComercial = datetime.date.today()
            if factura.fechaVistoBuenoTecnico == None:
                factura.fechaVistoBuenoTecnico = datetime.date.today()
            if factura.fechaVistoBuenoDirector == None:
                factura.fechaVistoBuenoDirector = datetime.date.today()
        ...
        self.wids['e_motivo_vto_bueno'].set_text(
            pclases.FacturaCompra.codigos_no_validacion[
                factura.get_codigo_validacion_visto_bueno()])
    ...
    def add_albaran(self, widget):
        ...
        if factura != None:
            ...
            if albaranes:
                ...
                #    factura.fechaVistoBuenoUsuario = datetime.date.today()
                #else:
            ...
            if model[iter].parent == None and model[iter][-1] != -1:
                ...
                utils.dialogo_info(titulo = "ERROR", 
                    texto = "Ha seleccionado un servicio.\n "
                            "Seleccione una línea de albarán para eliminar "
                            "el albarán de la factura actual.", 
                    padre = self.wids['ventana'])
        else:
            utils.dialogo_info('ERROR', 
                'Seleccione una línea de compra de la factura.\nSe eliminarán'
                ' todas las líneas pertenecientes al albarán de dicha línea.', 
                padre = self.wids['ventana'])
    ...
    def crear_nueva_factura(self, widget):
        ...
        idproveedor = utils.dialogo_combo(titulo = "SELECCIONAR PROVEEDOR", 
                                          ...
                                          ops = lista_provs, 
                                          padre = self.wids['ventana'])
        if idproveedor == None: return
        numfactura = utils.dialogo_entrada('Introduzca un número para la factura.', 'NÚMERO DE FACTURA', padre = self.wids['ventana'])
        if numfactura == None: return
        # proveedor = utils.combo_get_value(self.wids['cmbe_proveedor'])
        # if proveedor == None:
        #     idproveedor = None
        # else:
        ...
        facturas_existentes = [f for f in facturas_existentes if f.numfactura.upper() == numfactura.upper()]
        if facturas_existentes != []:
            ...
            utils.dialogo_info(titulo = "ERROR CREANDO FACTURA", 
                               texto = "La factura %s %s ya existe.\nNo puede crear una factura duplicada." % (numfactura, nombreproveedor), 
                               padre = self.wids['ventana'])
        else:
            if factura != None: 
            ...
            if proveedor != None:
                ...
                iva = 0.18
            factura = pclases.FacturaCompra(fecha = datetime.date.today(),
                                        ...
                                        fechaEntrada = datetime.date.today(), 
                                        iva = iva, 
    ...
    def buscar_factura(self, widget):
        ...
        if self.objeto != None:
            self.verificar_si_vencimientos()
        factura = self.objeto
        a_buscar = utils.dialogo_entrada("Introduzca número de factura:", titulo = "BUSCAR FACTURA", padre = self.wids['ventana']) 
        if a_buscar != None:
            resultados = pclases.FacturaCompra.select(pclases.FacturaCompra.q.numfactura.contains(a_buscar))
    ...
    def guardar(self, widget):
        ...
        try:
            ...
            factura.fechaEntrada = datetime.date.today()
        ...
        if (factura.vistoBuenoDirector 
           ...
           (factura.vistoBuenoDirector 
            ...
            utils.dialogo_info(titulo = "CAMBIOS EN FACTURA YA FIRMADA", 
                               texto = "Se van a guardar cambios en una "
                "factura ya autorizada.\nLos vistos buenos se reiniciarán "
                "debido a estos cambios.\nSerá necesario que se vuelva a "
                "firmar la factura antes de proceder al pago.", 
                               padre = self.wids['ventana'])
    ...
    def add_vto(self, boton):
        ...
        if not estimado:
            vto = pclases.VencimientoPago(facturaCompra = factura,
                    fecha = datetime.date(day = fecha[0], 
                                                     month = fecha[1], 
                                                     year = fecha[2]),
                    importe = cantidad,
                    observaciones = factura.proveedor and factura.proveedor.textoformapago or "")
                    ...
                    if (proveedor.diadepago != None 
                        ...
                        try:
                            ...
                            utils.dialogo_info(titulo = "ERROR DÍA ESTIMADO", 
                                               texto = "El proveedor tiene como día de pago un valor incorrecto: %s\nDebe usar únicamente números.\nCorrija este dato desde la ventana de clientes." % (proveedor.diadepago), 
                                               padre = self.wids['ventana'])
                    else:
                        diaest = False
                    for incr in vtos:
                        ...
                        vto = pclases.VencimientoPago(
                                fecha = factura.fecha + datetime.timedelta(days = incr),
                                importe = cantidad,
                                facturaCompra = factura, 
                                observaciones = factura.proveedor 
                                    and factura.proveedor.textoformapago or "")
    ...
    def cambiar_cantidad(self, cell, path, texto):
        ...
        if idvto > 0:   # Es -1 si no había.
            ...
            factura = self.objeto
            vto = pclases.VencimientoPago(fecha = datetime.date.today(),
                                           facturaCompra = factura,
                                           importe = cantidad)
        self.objeto.vencimientosConfirmados = False
        self.objeto.make_swap() # Para evitar falso positivo de actualización
    ...
    def cambiar_importe_pago(self, cell, path, texto):
        ...
        if idpago > 0:  # Es -1 si no había.
            ...
            factura = self.objeto
            pago = pclases.Pago(fecha = datetime.date.today(),
                                  facturaCompra = factura,
                                  importe = importe)
        self.rellenar_vencimientos()
    ...
    def add_linea_de_compra(self, boton):
        ...
        """
        # TODO: Esto no aumenta existencias a los productos. ¿Debería hacerlo?
        if self.objeto == None:
            return
        producto, texto_buscado = utils.pedir_producto_compra(self.wids['ventana'], self.objeto.proveedor)
        if producto != None:
            ldc = pclases.LineaDeCompra(productoCompra = producto, 
                                        pedidoCompra = None, 
                                        albaranEntrada = None, 
                                        facturaCompra = self.objeto, 
                                        cantidad = 0,       # Que lo ponga 
                                                    # el usuario después.
                                        precio = 0,         # Ídem
                                        descuento = 0,      # Ídem de ídem
                                        iva = self.objeto.iva,  # El IVA de 
                                            # la línea por defecto es el de 
                                            # la factura.
                                        entrega = '')
            self.objeto.vistoBuenoUsuario = True    # Si no lleva albaranes 
                                    # no necesita el visto bueno del usuario
            self.objeto.fechaVistoBuenoUsuario = datetime.date.today()
            self.actualizar_ventana()
        else:
            if utils.dialogo(titulo = "AÑADIR SERVICIO", 
                ...
                if concepto != None:
                    servicio = pclases.ServicioTomado(precio = 0, 
                                                facturaCompra = self.objeto, 
                                                descuento = 0, 
                                                cantidad = 1, 
                                                concepto = concepto, 
                                                ...
                                                #comision = None, 
                                                iva = self.objeto.iva)
                    self.objeto.vistoBuenoUsuario = True    # Si no lleva 
                        # albaranes no necesita el visto bueno del usuario
                    self.objeto.fechaVistoBuenoUsuario = datetime.date.today()
                    self.actualizar_ventana()
    ...
    def verificar_si_vencimientos(self):
        ...
        try:
            ...
            self.logger.error("facturas_compra::verificar_si_vencimientos -> Excepción: %s" % msg)
            pass
        if self.objeto.vencimientosPago != []:
            total = utils.parse_euro(self.wids['e_total'].get_text())
            totvencimientos = sum([v.importe for v in self.objeto.vencimientosPago])
                    ...
                    if not self.crear_vencimientos_por_defecto(None):
                        utils.dialogo_info(titulo = "VENCIMIENTOS NO CREADOS", 
                            texto = "Los vencimientos de la factura %s no se "
                                    ...
                                    "." % (factura.numfactura), 
                            padre = self.wids['ventana']) 
            # Compruebo que las fechas de los vencimientos son correctas, y si no, aviso.
    ...
    def adjuntar(self, boton):  # XXX: Código para adjuntos
        ...
        """
        #if self.objeto != None and utils.dialogo(titulo = "ALFA", 
        #                                         texto = "Funcionalidad en fase de pruebas. ¿Continuar?", 
        #                                         padre = self.wids['ventana'], 
        #                                         tiempo = 5):
        if self.objeto != None:
